...
## 🔵 队列（用户裁定 2026-08-04：一个一个做，不并行、不提前动手）

- [x] **① ✅ ACCEPTED + ARCHIVED 2026-08-04 — Gate 3 证据可重放性 advisory 检查** — commit `31a96aa`（**未 push**，由人决定）。Gate 4：Alex 独立重算 AC1–AC7 全 PASS（AC2/AC3 含补集断言 6/2·5/0·4/1；AC6 commit 血径恰三行；AC5 单哈希 `4c55bcb…`）；落地文本两树亲验；lite sentinel + lite SKILL 零改动；MECE drift 保留（canonical 7 / 镜像 6）；`config-quality.yaml` 零污染。Layer 2 两 reviewer P0=0（layer2-audit DISTINCT_COUNT=2 PASS）。**业务验收 dogfood**：新项自己的证据采集命令连跑两次 `cmp` = 0 diff。**Friction**：REVIEWER-TIER-DEGRADED（alias-mapped 无法 spawn 异模型；两 reviewer env 实证 `ANTHROPIC_MODEL=deepseek-v4-flash`）。KA 蒸馏 **3 条 L2**：ac-verification ×2（commit-血径 AC 与 `git add -A`/后续 commit 互斥；计数 AC 只约束总数不约束位置→补集断言，假 PASS 变体已被探针实证）+ gate-design ×1（聚合路由下 `model` 参数是请求非凭证，档位以 reviewer 实际自报为准）。归档：`.tad/archive/handoffs/{HANDOFF,COMPLETION}-20260804-evidence-replayability-check.md`。

### 🔴 兼容性 × token 浪费专项（用户裁定 2026-08-04：「要兼容各种模型和 harness 工具，不用非要调用强档」→ 找出同类规定全部改掉）
...
- [ ] **②e Blake 全文重读无界** — `blake/SKILL.md:514` `1. Re-read the selected handoff document (full content)`。配 100KB+ handoff 即 token 引擎。⚠️ **reviewer 那边已有界**（`blake/SKILL.md:1401` 模板规定只读 §6+§9 且禁止扩读），**Blake 自己无界**——又是同病治一半。
...
- [ ] **④ handoff 体量闸** — 生成时超 80KB / 20 AC → 提示拆分或降 Lite（提示不硬拒）。实测分布：出事那张 107KB/27AC；menu-snap 64KB、全屋智能 62KB、合规ai 52KB 都跑完了；其余 <40KB。落点必须在**任何 reviewer 被 spawn 之前**（step1b/step1c 之间），否则"坚持原样"是唯一理性选项。

**📌 模式记录（不是待办，是判断依据）**：②③ 是同一天发现的**同形缺口**——lite 有护栏、full 没有。lite 是踩完坑之后建的，full 是更早的设计。下次遇到 full 侧的纪律问题，**先去 lite 找有没有现成答案**，别从头设计。

- [x] **Codex Universalization 三单计划 ✅ COMPLETE 3/3 — ALL ACCEPTED + ARCHIVED + PUSHED 2026-08-03** — Step 3 词汇通用化单 ✅ Gate 4 ACCEPTED（commits `39ba1c1` baseline → `77c573d` feat → `0d71100`+`a01603b` gate 记录）：能力档位表（旗舰推理档 + reasoning≥high + route=unknown 保守三选一）、Codex/other Model 捕获（CODEX_HOME + per-agent toml + host-only 脱敏）、11 文件平台绑定交互条款（含 agent-computer-interface SAFETY 硬门裁决入集）、full 三副本 reviewer 首行 model 自报、§E 三处 sonnet tiering、`.agents` parity。Gate 2 双专家两轮 4 P0 dispositioned；Gate 3 三 distinct reviewers 终版 0/0/0；Gate 4 Alex 独立重算 AC1–10 全 PASS + 落地文本亲验（rg 禁令生效，环境假 FAIL 绝迹）。KA 蒸馏 1 条 L2（ac-verification #20：文本锚对解析器级回归全盲——YAML front-matter 插入事故 + 字面命令测试 + 块序数归属）。Archived: `.tad/archive/handoffs/{HANDOFF,COMPLETION}-20260803-model-vocabulary-universalization.md`。**三单 follow-ups 汇总**：①认证 re-probe（scratch CODEX_HOME 补 auth.json 重测 Spike C/D/E）②step1/2 旧 AC 脚本去 rg ③workflow 既有 top-level return 卫生 ④pre-gate-check find 深度 ⑤layer2-audit 收编 safety-architecture/acceptance-repro reviewer 名 ⑥REVIEW_SCHEMA 加 model 字段 ⑦下游项目需重跑安装器换 hooks.json。 — Step 1 战果：Codex hooks 接线复活（`{description,hooks}` schema 双 spike 实测）、36 处 reference 相对化 + parity 前置守卫（quote-agnostic 双树探针）、台账 0.146 全量合法重验（runtime-freshness 21 PASS/0 BLOCK exit 0）；Gate 4 Alex 独立重算全绿（AC3 rg→grep 等价）；commits c43d8e3+e73a3c8 未推送（推送提示：下游需重跑安装器换 hooks.json）；KA 蒸馏 2 条 L2（ac-verification exit-0 两分支范式、release-sync early-exit 盲区）。Step 2 = `HANDOFF-20260803-codex-knowledge-ingress.md` ✅ Gate 4 ACCEPTED+ARCHIVED 2026-08-03（commits 55751de+6893d6c+db56d74 未推送; AGENTS.md 知识入口/Critical Rules + hook 信封归一化 + honest PreCompact + detect-platform 信号表 + Source headers 38→0 + brain-index 兜底；Alex 独立重算全绿，AC-07 rg→grep 等价二次出现已蒸馏；台账 context_compaction 修为 verified_partial；**三个 spike 因 401 全部 honest-partial（信封投递/PreCompact 触发/harness 信号均未实测，落最保守分支），用户已批准三项 DEGRADED_WITH_APPROVAL**；KA 蒸馏 2 条 L2：ac-verification 凭证剥离 spike 范式 + shell-portability rg-127 假 FAIL）。**Follow-ups**：①认证 re-probe（scratch CODEX_HOME 补 auth.json 重测 Spike C/D/E）②AC 脚本 rg 消除 ③full 通道 reviewer model 自报条款（→ step 3）。 — 起因：用户纠偏（lite 初衷 = 减重去仪式 + TAD 必须 harness/model 通用），全库耦合扫描（Explore agent，61 tool-uses）发现 top-10 gap，其中 3 处**现在就坏**：①`.codex/hooks.json` schema 被 codex ≥0.145 拒收且 tad.sh 每次安装再生成坏文件；②`.agents` 镜像 36 处 `reference: ".claude/skills/…"` 在 codex-only 安装上全部悬空（= "Codex dogfood Blake skipped Layer 2" 事故根因）；③runtime-compat 台账钉在 0.137、hooks 行已为事实错误（漂移仪表红灯报绿）。计划：**step 1 止血单**（hooks schema spike-first + reference 相对路径化 + parity fail-closed 新检查 + 台账重验 0.146 + detect-platform 误判修复）→ `HANDOFF-20260803-codex-wiring-stopbleed.md`（Gate 3 PASS，待 Alex Gate 4）；**step 2 知识入口单**（AGENTS.md 知识入口/Critical Rules + hook 信封归一化 + honest PreCompact + detect-platform 信号表 + Source headers 38→0 + brain-index 兜底）；**step 3 词汇通用化单**（Reviewer 档位改能力档位表 + Model 捕获三分支 + AskUserQuestion 抽象为平台绑定交互决策）。独立议题另记：Route Contract R0-R3 仪式减重（lite 回潜最重一块，两套机制管一个通道判定）；Workflow 编排移植（前提 = 前三单）。扫描完整报告在会话记录；已做对不重做：lite 共享记忆契约、AGENTS.md lite 路由、Model 行格式、portable-rules 替换表。

- [x] **Lite Review Hardening ✅ ACCEPTED + ARCHIVED 2026-08-03** — Added mandatory execution probes, reviewer tier rules, cross-role disambiguation/refusal recording, and verifier `model` enforcement across byte-identical Lite mirrors. Gate 4: 9/9 AC independently rerun by Alex, sentinels/mirrors/redlines intact, AC8 probe substantive (comment-READY bait caught by real execution), verifier PASS, commit scope clean (48e3112 + e5b810d, NOT pushed). First single executed on Codex/gpt-5 with Model discipline live. KA distilled: 2 L2 entries (ac-verification "Section-Scoped Checks Must Share the Governed Location's Exact Boundary Semantics"; shell-portability "ugrep dash-leading patterns require -e"). Archived: `.tad/archive/handoffs/{HANDOFF,COMPLETION}-20260803-lite-review-hardening.md`. Carry: its Reviewer-tier vocabulary (opus/fable) is Claude-coupled by construction → fixed in step 3 above.

- [ ] **Lite review + multi-model portability verification — DISCUSSED + RESEARCHED 2026-08-02, upgrade plan queued** — Full-day *discuss session: lite direction review → 7 blind spots → portability research (no-memory verification). **Landmark finding (log-verified): deepseek-v4-flash executed the ENTIRE lite quality chain in production** — smart-home singles wake-sensitivity (185 turns) + reminder-reliability (401 turns, 11 ACs, F0 routing catch, reviewer FAIL→PASS chains, 28 unit tests, honest four-state) ran 100% on v4-flash incl. subagent reviewer spawns; web-reported "DeepSeek never spawns subagents" did NOT reproduce under TAD structure (hypothesis: named BLOCKING reviewer step + mechanical downstream checks force it — CAAF "assertions + process scaffolding" reproduced in production). Also verified: Codex automation layer now equivalent-or-stronger (hooks stable v0.124+, plugins/marketplace, sub-agent addressing, Starlark exec policy — my 2026-06 memory was stale); SKILL.md is a cross-vendor open standard (symlink-able across .claude/.agents); DeepSeek-in-Claude-Code officially documented by DeepSeek (opus→v4-pro, sonnet/haiku→v4-flash). Report: `.tad/evidence/research/2026-08-02-multi-model-portability-verification.md`. **Upgrade waves queued:**
  - Wave 1 (now): (a) `Model:` field → **✅ ACCEPTED + ARCHIVED + COMMITTED 2026-08-02** (user "commit吧"; role-violation incident logged + ratified "下不为例" — see violations.log + memory 3rd-occurrence entry; protocol gap "sanctioned single-session cross-role authorization form" queued for lite amendments): `HANDOFF-20260802-model-provenance-field.md` (express; Gate 2: 2 experts, 17 findings dispositioned; user-authorized direct execution "你自己做你啊" DEGRADED_WITH_APPROVAL; 6 files changed, 8/8 AC green, impl review round 1 CONDITIONAL 2 P1 — comment-block blinded the writer required-field guard [mutation-proven] + alex-lite missing value format — both fixed, incremental re-review **PASS** with byte-for-byte artifact reproduction; follow-up queued: verify-state-flow.sh required_fields +model). Completion appended in handoff; uncommitted by policy; (b) model-diversity audit → **DONE 2026-08-02, verdict decisive**: strong-model blind audit (execution-probe-verified) returned **FAIL (P0×1+P1×6+P2×12)** on the flash-chain GATE-PASS single. flash caught bookkeeping/surface defects (its 5 P1 were real), missed systemic ones: retry-budget/delivery-time ratio P0 (healthy-system silent reminder loss — **LIVE IN PRODUCTION on homeserver now**, incl. medication reminders), fail-open dedup ledger, missing flock, batch-abort amplification, announce=ok honesty defect. Two confounded factors both actionable: model strength AND verify-by-execution method. Ruling: cheap-model execution OK; production-critical L3 review needs strong model + mandatory execution probes. Report: `.tad/evidence/research/2026-08-02-model-diversity-audit-results.md`; full audit copied to smart-home project `.tad/evidence/research/2026-08-02-external-blind-audit-reminder-reliability.md`. 后续修复属 smart-home 项目自身 backlog（用户裁定 2026-08-03：跨项目工作项不记录在 TAD）。Gate-escape data point logged for gate-roi tracking.
  - Wave 2 (short-term): (c) U1 symlink spike — retire byte-identical mirror pairs if tad.sh/downstream installs tolerate symlinks; (d) Codex debt: `$alex-lite` registration + `$alex`/`$blake` invocation degradation diagnosis (existing backlog); (e) re-measure lite cycle cost against 45K falsification threshold (now 3 cost cells: Claude/Codex/DeepSeek); (f) dead-doc cleanup from 7-27 assessment (cheap, high ROI).
  - Wave 3 (after 2-3 more smart-home singles): (g) formalize "lite contract + full discipline" as the official escalation path (define execution_depth=standard/full operational meaning — today's rev-2 improvised authorization becomes the standard form); (h) refine F0 `production` match breadth (user-owned reversible service restarts vs irreversible external ops — smart-home routine tasks currently all route to full execution); (i) platform/model tags on platform-specific knowledge entries.
  - Blind spots log (for audit trail): lite bloat + stale cost number; mirror entropy; Codex-side lite unreachable; full-channel dead docs untouched; lite-discoveries → *accept distillation unwired (existing follow-up); N1 F0 breadth; N2 undefined hybrid escalation; N3 voice-studio built informal lite (E26 one-pagers) without the channel — strongest adoption signal + gap.

- [x] **TAD Lite v1.1 质量修订 ✅ PUBLISHED as v2.36.0 — 2026-07-30**（commits 7db2542 feat + 2d2bf9b release 已推 origin/main + annotated tag v2.36.0 远端确认；五门全绿 parity/version/sweep/migration/denylist；version 门抓到 3 处我新写的 stale 引用并已改措辞）。发布时待办 (a) README/tad-help lite 描述 + 成本数字 ✅ 本次已兑现（~35K 标注为估算）；(b) Codex AGENTS.md 零 lite 路由、(c) `$alex-lite` Codex 路由未注册 → 仍在 backlog。 — Gate 4: AC1-16 Alex 独立重算 16/16 零偏差 + AC17 行为 dogfood DEGRADED_WITH_APPROVAL PASS（用户原话"你自己完成吧"；探针 A 真 spawn L2.5 reviewer 且在玩具契约上抓到真 P1 `grep -c .` 空行逃逸、修复后增量复核 PASS；探针 B 负控硬停）。报告：`.tad/evidence/acceptance-tests/lite-v11-quality-amendments/gate4-acceptance-report.md`。留观 P2：L2 模板占位段的恢复误判边缘（blake 条数比对兜住）。**Carry-to-first-real-use**：下游项目首次真实使用 v1.1 lite = 真正现场验证。— 首个真实 LITE 单（全屋智能化 calendar-write）复盘 + 质量对照研究（`.tad/evidence/research/2026-07-30-lite-vs-full-quality-comparison.md`）驱动的 12 项修订：核心 = alex-lite 新增 L2.5 每单契约 reviewer（用户拍板"每单都审"，设计侧 1 + 实现侧 L3 对称）+ blake L0.5 降为机械复查 + 目标锚/方案速写/六条件自治修复/七态状态词/单步验收/follow-up 强制/Series 锚点/express 消歧/opt-in 复盘。Handoff: `HANDOFF-20260730-lite-v11-quality-amendments.md`（v2，R1 双专家 10 P0 全处置）。**发布时待办（本单不做，*publish 时一并）**: (a) README L90-91 + tad-help L223 的 lite reviewer 结构描述更新（"实现后 1 reviewer"→"契约+实现各 1"）+ token 数字以实测 dogfood 定数（估 ~35K 非 23K）; (b) Codex `.tad/codex/`/AGENTS.md 零 lite 路由内容（CR-P2-23，backlog）; (c) `$alex-lite` Codex 路由未注册（codex/README.md L151-154，与既有 $alex/$blake 退化诊断同批）。

- [x] **TAD Lite channel ✅ PUBLISHED as v2.35.0 — 2026-07-30** (4 commits 34865ba→065c19a pushed to origin/main + annotated tag v2.35.0; all gates green: parity/version/sweep/migration/denylist — denylist gate caught + fixed real tad.sh drift: `dependencies` missing from inlined DENY_LIST since 2026-07-14, would have overwritten downstream user registries; Gate 3 PASS 2 distinct reviewers, Gate 4: 14 ACs independently recomputed by Alex — identical) — `/alex-lite` (81L) + `/blake-lite` (115L) self-contained skills + CLAUDE.md §1/§2.5/§3 routing (+10/-0) + `.agents` byte-identical mirrors + Codex smoke test PASS (codex-cli 0.145.0). **Dogfood measured ~23K/cycle vs full 300K-1M** (51% of 45K target; falsification threshold not triggered). Design survived 3 review rounds (R1: 12 P0 from 2 experts; R2 incremental: 1 STILL-BROKEN + 9 fix-introduced defects — round-2-catches-fix-defects pattern reconfirmed). New L2 pattern: gate-design "Independent Perspective Lives in Clean Context, Not Terminal Boundaries". **Follow-ups queued:** (a) wire `*accept` distillation to read `.tad/evidence/journal/lite-discoveries.md` (needs full-TAD handoff, touches acceptance-protocol); (b) full `$alex`/`$blake` Codex invocation degradation diagnosis (protocol-class task — NOT a lite candidate per valve); (c) 2 known P2s: archived-LITE re-entry edge + §4 implicit scoping; (d) observe lite usage for express-merge decision + drift audit via `ls .tad/archive/handoffs/LITE-*`.
...
- [x] **Evidence Replayability Check ✅ GATE 3 PASS 2026-08-04 (commit 31a96aa, NOT pushed)** — Gate 3 advisory check added to canonical (.tad/gates/gate-canonical-checklist.md) + mirrors (.claude/.agents skills/gate, parity-generated). AC1-7 PASS; Layer 2: 2 distinct reviewers (spec-compliance + code-reviewer) both PASS 0 P0 (probe reproduced+ killed Gate-2-R1 fake-PASS variant); new advisory item dogfooded itself (evidence rerun 0 diff). DEGRADED_WITH_APPROVAL: reviewer tier (alias-mapped v4-flash, user-authorized, per-reviewer self-reported Model). Awaiting Alex Gate 4 acceptance.
