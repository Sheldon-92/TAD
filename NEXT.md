...
- [ ] **Pack install layer is platform-blind (quantified 2026-06-10, supersedes old item (a))** — `install.sh` writes only `.claude/skills`; `.agents` keeps verbatim master copy. Measured on synced projects: **6 packs' SKILL.md install-transformed on .claude side only** (academic-research / ai-agent-architecture / ai-voice-production / video-creation / web-frontend / web-ui-design) **+ ml-training exists only on .claude side** (no pre-built SKILL.md, install-generated). Codex users of these 7 packs in downstream projects get raw/missing editions. Fix direction: platform-aware install.sh OR post-install mirror `.claude/skills/{pack}` → `.agents/skills/{pack}`. Framework change → handoff to Blake next session. (Structural-gate side is already handled: gate runs pre-transform.)
