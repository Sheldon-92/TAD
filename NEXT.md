...
- [ ] **EPIC: Capability Pack Quality Leveling (24 packs, 双层尺)** — Phase 1/6 ✅ **Gate 4 ACCEPTED 2026-06-13** (commit f2addac; Alex 独立重算 AC2=24 + 两层 negative control 真 FAIL: Layer A 0/10 / Layer B specN=0→1/5; Layer 2 ×3 PASS; handoff archived; Gate 4 report `.tad/evidence/acceptance-tests/pack-quality-phase1-bar-baseline/`). **Next: Phase 2 = Batch 1 (7 包: ml-training*/data-engineering/ai-podcast-production*/agent-memory/agent-orchestration/knowledge-graph/ai-tool-integration; *=补 fixture) — 待用户启动.** Delivered: `QUALITY-BAR.md` (双层尺 + 2 negative control 实跑 FAIL) + `BASELINE-AUDIT.md` (24 包评分 + 弱→强 4 批 7/5/5/4, golds 排除) + NotebookLM `capability-pack-meta-design`. MQ1 纠偏: 2 个无 fixture 包 (ml-training + ai-podcast-production). Layer 2 ×3 PASS. KA: pack-evaluation.md '结构-gold ≠ 深度-gold'. **Next: Alex Gate 4 → 接受则 Phase 2 (Batch 1: ml-training/data-engineering/ai-podcast-production/agent-memory/agent-orchestration/knowledge-graph/ai-tool-integration).** Epic: `EPIC-20260613-capability-pack-quality-leveling.md`. Aligns O2/KR1.
