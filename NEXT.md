...
- [ ] **memory-redirect-capture-layer — GATE 3 PASS 2026-07-12, awaiting Alex Gate 4** (Blake commits 7fbe24e + c7682e2, NOT pushed; AC1-AC7/AC9/AC10 all verified, AC6 scope-PASS with global-parity friction; Layer 2: spec-compliance + code-reviewer + security-auditor all PASS) — COMPLETION: `.tad/active/handoffs/COMPLETION-20260712-memory-redirect-capture-layer.md`. Gate 4 specials: AC8 human new-session test, D1 selective-git confirm, 7-file SENSITIVE triage review, global parity re-run pre-*publish, parity-tool bugfix handoff (local/ mirror + concurrency). Implements DR-20260712 verdict 1.
...
- [ ] **Surplus Burn wind-down — RE-PRIORITIZED 2026-07-12 (user-approved; surplus premise gone with new billing cycle)** — Full record: `.tad/active/session-state.md`. DONE 2026-07-12: 4 deliverables merged (repositioning 7d03d96 + save-workflow e62c7b4 + save-skill 259b0e6 + .agents mirror b12e518; tad.sh glob-arm fix salvaged from forgotten 07-02 dogfood worktree — closes Deferred item); 4 completed handoff sets archived; 3 handoffs cancelled with taxonomy (o1-landscape pivoted / tad-self-test scope-change / session-health-check obsolete); 6 worktrees removed. NEW PRIORITY ORDER: ① gate-roi-measurement — ✅ ACCEPTED + ARCHIVED 2026-07-12 (commit ca66e85, Gate 4 human-confirmed): **net-positive → GO** — 27-sample catch-agnostic frame, STRICT pre-registered rule cleared at NC%=63% (bar 25%) / P01=91; 6/27 zero-catch rows; dual-blind FR8 9/10; cost side unmeasured by design → follow-on = cost-side measurement then revisit 2026-04-15 mechanical-enforcement; report `.tad/evidence/research/gate-roi-measurement-2026-07.md`; new L2 pattern Dual-Blind Disjoint-Row Spot-Check → ② skill-body-reference-boundary (long-standing SAFETY P0) → ③ small real-debt fixes (deprecate-domain-pack-yaml rerun READY; detect-state fixture P0; multi-table-parser; classify-scope; express-frontmatter marker) → ④ ready-shelf handoffs when picked (tad-methodology-skeleton > pack-behavioral-examples-scaffold > codex-adapter-yaml). Dropped: 18-casualty batch relaunch (synth filler). System lessons → 3 fix-candidate handoffs still pending: yolo-epic worktree-visibility false-FAIL (hit 3x + confirmed again at merge verification), surplus-execute success-criterion ignores impl P0s, Workflow args quirk.
