# Handoff Template: Create New Domain Pack — Mobile Development
...
为 **Mobile Development** 创建一个 Domain Pack — 一个 YAML 配置文件，让 TAD 能在这个领域"做事"。
...
执行这 5 条 WebSearch（把 mobile-development 替换为具体领域）：

```
1. "GitHub claude skills mobile-development SKILL.md"
2. "GitHub AI agent mobile-development skill prompt framework"
3. "GitHub awesome claude skills mobile-development"
4. "mobile-development best practices checklist framework"
5. "site:github.com mobile-development SKILL.md OR CLAUDE.md"
```
...
```markdown
# mobile-development Skills Best Practices
...
**写入**：`.tad/spike-v3/domain-pack-tools/mobile-development-skills-best-practices.md`
...
Agent({
  ...
  读取 .tad/domains/mobile-development.yaml 的 {capability} 部分，按 steps 逐步执行。
...
| # | 文件 | 检查 |
|---|------|------|
| 1 | `.tad/spike-v3/domain-pack-tools/mobile-development-skills-best-practices.md` | ≥3 仓库 × 5 维度 |
| 2 | `.tad/spike-v3/domain-pack-tools/mobile-development-tool-research.md` | ≥2 工具实测 |
| 3 | `.tad/domains/tools-registry.yaml` 更新 | 新工具有完整条目 |
| 4 | `.tad/domains/mobile-development.yaml` | 四层 steps + 质量标准 |
...
## Alex 的领域补充：Mobile Development 特定指引

### 范围定义

**Mobile Development Pack 做移动端代码实现。输入是 Mobile UI Design 的产出。**
- ✅ 项目初始化（React Native / Expo / Swift / Flutter 脚手架）
- ✅ 原生组件实现（导航、列表、表单、手势处理）
- ✅ 状态管理（React Native: Zustand/Redux, Swift: SwiftUI @State/@Observable）
- ✅ API 集成（REST/GraphQL + 离线缓存 + 网络状态处理）
- ✅ 平台特性（推送通知、相机、位置、生物识别）
- ✅ 性能优化（列表虚拟化、图片缓存、包体积、启动速度）
- ✅ 代码质量（Lint、类型检查、平台兼容性）
- ❌ UI 设计（mobile-ui-design pack）
- ❌ E2E 测试（mobile-testing pack — 未来）
- ❌ App Store 提交（mobile-release pack — 未来）

### 用户当前技术栈

从 Menu Snap 项目推测（需 Phase 1 确认）：
- **框架**: React Native / Expo（最可能，和 Web 共享 React 知识）
- **或**: Swift/SwiftUI（如果是纯 iOS）
- Blake 在 Phase 1 研究时应确认用户实际用什么框架

### 建议的 Capabilities

| Capability | 做什么 | 关键产出 | 工具 |
|-----------|--------|---------|------|
| `project_scaffold` | 项目初始化 + 目录结构 + 配置 | 可运行的项目骨架 | expo CLI / react-native CLI / xcode CLI |
| `native_components` | 实现原生 UI 组件（导航、列表、表单） | 组件代码 | Write + 平台 CLI |
| `state_management` | 状态管理方案设计与实现 | Store/Hook 代码 | Write |
| `api_integration` | API 调用 + 离线缓存 + 网络状态 | API 层 + 缓存逻辑 | Write |
| `platform_features` | 推送通知 / 相机 / 位置 / 生物识别 | 平台集成代码 + 权限配置 | expo-* 包 / native modules |
| `performance` | 列表虚拟化、图片缓存、启动优化、包体积 | 优化后 build + 分析报告 | metro bundler, Xcode Instruments |
| `code_quality` | Lint + 类型检查 + 平台兼容性 | 零错误代码 | ESLint, TypeScript, Xcode build |

### 和 Web Frontend 的关键区别

| 维度 | Web Frontend | Mobile Development |
|------|-------------|-------------------|
| 框架 | Next.js / Vite | **Expo / React Native / Swift** |
| 导航 | React Router | **React Navigation / UINavigationController** |
| 列表 | 普通 div | **FlatList / VirtualizedList**（性能关键） |
| 存储 | localStorage | **AsyncStorage / SQLite / Keychain** |
| 网络 | 假设在线 | **离线优先 + 网络状态检测** |
| 权限 | 几乎不需要 | **相机/位置/通知/生物识别 需要声明** |
| 构建 | npm run build | **expo build / xcodebuild / gradle** |
| 调试 | DevTools | **Metro + Flipper / Xcode Instruments** |

### Phase 1 搜索关键词

```
"GitHub claude skills React Native Expo mobile SKILL.md"
"GitHub claude skills iOS SwiftUI mobile development"
"GitHub cursor rules React Native best practices"
"GitHub AI agent mobile app code generation"
"GitHub awesome claude skills mobile development"
"React Native performance optimization checklist 2026"
"Expo CLI workflow best practices"
"iOS app development quality checklist"
...
| 类别 | 工具 | 说明 | registry 有？ |
|------|------|------|-------------|
| 脚手架 | Expo CLI | `npx create-expo-app` | ❌ 需新增 |
| 脚手架 | react-native CLI | `npx react-native init` | ❌ 需新增 |
| iOS 构建 | xcodebuild | `xcodebuild -workspace ... -scheme ...` | ❌ 需新增 |
| Android 构建 | gradle | `./gradlew assembleRelease` | ❌ 需新增 |
| Lint | ESLint (已有) | React Native 配置不同于 Web | ✅ 部分复用 |
| 类型检查 | TypeScript (已有) | 同 Web | ✅ 复用 |
| 包分析 | react-native-bundle-visualizer | 包体积分析 | ❌ 需查 |
| 性能 | Flipper | 移动端性能调试 | ❌ 需查 |
| 模拟器 | xcrun simctl | iOS 模拟器控制 | ❌ 需新增 |
| 测试 | jest (已有) | 单元测试 | ✅ 部分复用 |

**注意**：Expo CLI 是核心工具（如果用户选 Expo）。xcodebuild 是 iOS 原生构建的核心。

### 和 Mobile UI Design 的衔接

```
Mobile UI Design 产出:           Mobile Development 输入:
├── 平台规范摘要           →     知道遵循 HIG 还是 Material
├── 导航流程图             →     React Navigation 配置
├── 线框图 (HTML/Figma)    →     组件结构参考
├── Design Tokens          →     主题配置（颜色/字体/间距）
├── 手势交互规范           →     手势处理实现
├── 组件清单               →     开发任务列表
└── 可用性审查报告         →     触控目标/可达性要求
```
...
**"Menu Snap iOS 版的前端实现"** — 和 UI Design 用同一议题验证衔接。
1. Expo 或 React Native 初始化
2. 实现主要界面（菜单浏览 + 拍照识别 + 结果展示）
3. API 集成（连接后端）
4. ESLint + TypeScript 零错误
5. expo build 成功

### ⚠️ 产出是代码 — 代码类 step 模型

和 web-frontend 类似，大部分 capability 用**选型→执行→验证→优化**模型：
- `project_scaffold`: 选型（Expo vs RN vs Swift）→ 执行（npx create-expo-app）→ 验证（expo start 成功）→ 优化（配置精简）
- `performance`: 选型（优化方案）→ 执行（实施优化）→ 验证（启动时间/包大小测量）→ 优化（进一步精简）

quality_criteria 可自动化验证：
- `expo start` 成功
- `npx tsc --noEmit` 零错误
- `npx eslint .` 零错误
- `expo build` 成功
