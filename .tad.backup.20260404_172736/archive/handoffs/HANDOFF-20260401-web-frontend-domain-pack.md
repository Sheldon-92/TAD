...
- ✅ 代码质量（Lint、类型检查、代码审查）
- ❌ UI 设计（那是 web-ui-design pack）
- ❌ 后端 API 实现（那是 web-backend pack）
- ❌ E2E 测试（那是 web-testing pack）
- ❌ 部署（那是 web-deployment pack）

### 建议的 Capabilities

| Capability | 做什么 | 关键产出 | 可能的工具 |
|-----------|--------|---------|-----------|
| `project_scaffold` | 项目初始化（框架选型、目录结构、配置文件） | 可运行的项目骨架 | create-next-app, create-vite, shadcn CLI |
| `component_development` | 实现 UI 组件（从设计稿到代码） | React/Vue 组件文件 | Write (内置), shadcn add |
| `state_management` | 状态管理方案设计与实现 | Store/Context/Hook 代码 | Write (内置) |
| `api_integration` | 前端调用后端 API（数据获取、错误处理） | API 调用层代码 + 类型定义 | Write (内置), openapi-typescript CLI |
| `styling` | 样式实现（Tailwind/CSS Modules、响应式、暗色模式） | CSS/Tailwind 配置 + 组件样式 | Tailwind CLI, PostCSS |
| `build_optimization` | 构建优化（Bundle 分析、代码分割） | 优化后的 build + 分析报告 | vite build, webpack-bundle-analyzer |
| `code_quality` | 代码质量检查（Lint、类型、格式化） | 零错误的代码 | ESLint, TypeScript tsc, Prettier |
...
"GitHub AI agent frontend code generation"
"GitHub awesome claude skills frontend web development"
"React component development checklist 2026"
"frontend code quality best practices AI coding agent"
"GitHub shadcn CLI automation"
...
| 类别 | 搜索什么 | 说明 |
|------|---------|------|
| 脚手架 | create-next-app, create-vite, create-t3-app | 项目初始化 CLI |
| 组件库 | shadcn CLI (`npx shadcn@latest add`) | AI 友好的组件库 |
| 类型生成 | openapi-typescript CLI | 从 OpenAPI spec 生成 TypeScript 类型 |
| Lint | ESLint (`npx eslint .`) | 代码规范检查 |
| 类型检查 | tsc (`npx tsc --noEmit`) | TypeScript 类型检查 |
| 格式化 | Prettier (`npx prettier --check .`) | 代码格式化 |
| Bundle 分析 | vite-bundle-visualizer, webpack-bundle-analyzer | 打包分析 |
| 性能 | Lighthouse CI (`npx lhci`) | 性能分数 |

**大多数工具通过 npx 免安装运行 — 非常适合 CLI-first 策略。**

### 和 UI Design Pack 的关系

```
Web UI Design pack 产出:              Web Frontend pack 输入:
├── 线框图 (HTML/Figma)         →     组件结构参考
├── Design Tokens (CSS vars)    →     Tailwind/CSS 配置
├── 交互规范                    →     状态管理设计
├── 响应式断点规范              →     Media query / Tailwind breakpoints
└── 组件清单                    →     组件开发任务列表
```

### 测试议题建议

**"实现一个 Todo App 的前端"**（和 UI Design pack 用同一个议题，验证两个 pack 能衔接）：
...
### ⚠️ 关键区别：这个 Pack 的产出是代码

和 Product Definition（产出文档）、UI Design（产出设计稿）不同，Frontend Pack 的产出是**可运行的代码**。

这意味着：
- quality_criteria 可以自动化验证（`tsc --noEmit` 零错误、`eslint .` 零错误、`npm run build` 成功）
- 工具不只是"生成文件"，还包括"检查文件"（lint、type check、build）
- E2E 测试可以实际运行应用并检查行为
