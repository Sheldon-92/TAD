# Handoff Template: Create New Domain Pack — HW Firmware
...
为 **HW Firmware** 创建一个 Domain Pack — 一个 YAML 配置文件，让 TAD 能在这个领域"做事"。
...
执行这 5 条 WebSearch（把 hw-firmware 替换为具体领域）：

```
1. "GitHub claude skills hw-firmware SKILL.md"
2. "GitHub AI agent hw-firmware skill prompt framework"
3. "GitHub awesome claude skills hw-firmware"
4. "hw-firmware best practices checklist framework"
5. "site:github.com hw-firmware SKILL.md OR CLAUDE.md"
```
...
```markdown
# hw-firmware Skills Best Practices
...
**写入**：`.tad/spike-v3/domain-pack-tools/hw-firmware-skills-best-practices.md`
...
Agent({
  ...
  读取 .tad/domains/hw-firmware.yaml 的 {capability} 部分，按 steps 逐步执行。
...
| # | 文件 | 检查 |
|---|------|------|
| 1 | `.tad/spike-v3/domain-pack-tools/hw-firmware-skills-best-practices.md` | ≥3 仓库 × 5 维度 |
| 2 | `.tad/spike-v3/domain-pack-tools/hw-firmware-tool-research.md` | ≥2 工具实测 |
| 3 | `.tad/domains/tools-registry.yaml` 更新 | 新工具有完整条目 |
| 4 | `.tad/domains/hw-firmware.yaml` | 四层 steps + 质量标准 |
...
### 范围
- ✅ 固件架构设计（主循环、中断、状态机）
- ✅ 外设驱动（OLED、E-ink、触觉、触摸、WiFi、BLE）
- ✅ 低功耗设计（Deep Sleep、RTC 唤醒、功耗测量）
- ✅ OTA 更新设计
- ✅ 构建 + 上传（arduino-cli / PlatformIO）
- ✅ 代码质量（内存管理、中断安全、看门狗）
- ❌ 电路设计（hw-circuit-design pack）
- ❌ 外壳（hw-enclosure pack）

### 建议 Capabilities
| Capability | 做什么 | 工具 |
|-----------|--------|------|
| `firmware_architecture` | 主循环设计 + 状态机 + 任务调度 | D2 |
| `peripheral_drivers` | 外设初始化 + 驱动封装 | arduino-cli |
| `low_power_design` | Deep Sleep + RTC + 功耗预算 | arduino-cli |
| `communication` | WiFi/BLE/MQTT 通信协议 | arduino-cli |
| `build_upload` | 编译 + 上传 + 版本管理 | arduino-cli, platformio |
| `firmware_testing` | 串口调试 + 单元测试 + 集成测试 | serial monitor |
| `firmware_documentation` | 引脚映射 + API 文档 + 数据流图 | Typst, D2 |

### 用户已有基础
`_template/` 提供：WiFi + Haptic + Touch + Sprite 的可复用模板。HeartBeatZoo 的数据驱动架构（动物 struct + per-animal timing）。

### 测试议题
**"Wayo 固件开发"** — Deep Sleep + RTC + E-ink 刷新 + Movebank API 数据
