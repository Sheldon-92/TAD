...
#   --zero-touch    the 12 category-A "preserve target's own" dir names, LC_ALL=C sorted.
...
# DENY_LIST = category-A (zero-touch, 12) + category-C (transient/main-only, 5) = 17 dirs.
...
#
# Category A — zero-touch (preserve each target's own copy; NEVER sync, 12 dirs):
