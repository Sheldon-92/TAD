...
parse_safety_counts() {
  awk '
    /^[a-z_]+:/ {
      ...
      sub(/:.*/, "", section)
      delete counts
      next  # P1-2 fix: skip pattern matching on the header line itself
    }
...
# Feature markers: read from parity-criterion.md (self-sustaining — add new markers there)
feature_markers=""
if [ -f "$PIN_FILE" ]; then
  feature_markers=$(awk '/<!-- FEATURE:/ { sub(/.*<!-- FEATURE:/, ""); sub(/ -->.*/, ""); print }' "$PIN_FILE")
fi
if [ -z "$feature_markers" ]; then
  feature_markers="deliverable research_complexity step4_5"
  echo "  (fallback: using built-in marker list)"
fi
