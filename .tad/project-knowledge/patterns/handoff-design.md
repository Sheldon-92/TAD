...
### `git grep` Is Blind to Untracked Files — a Consumer Scan That Uses It Alone Will Miss Live Sibling Contracts - 2026-08-05
- **Context**: Drafting the routing-machinery removal (EPIC-20260804 P2+P3). Alex ran a consumer scan for every concept being deleted and instructed the Gate-2 reviewers to "use `git grep` across the whole repo, do not hand-pick directories" — deliberately, because a previous single had been burned by hand-picked search paths. Two review rounds and four reviewers still missed a live sibling contract sitting in `.tad/active/handoffs/`.
- **Discovery**: `git grep` searches **tracked** files only. In this framework every in-flight artifact — active handoffs, the current Epic, session state — is deliberately untracked until acceptance, so the entire set of documents most likely to conflict with a protocol change is invisible to the very command chosen for thoroughness. The miss was not academic: the sibling contract's AC pinned an md5 of the exact block this single rewrites, so it was **green before and red after**, and its scope note ("lite keeps its own tier enforcement, this single does not touch it") was falsified by the new spec. Only `git ls-files --others --exclude-standard` surfaced it. Generalizes: any "did I catch every consumer" sweep must union tracked and untracked, and must treat *other in-flight contracts* as consumers — not just documentation and code.
- **Action**: Consumer/impact scans run **two** commands, never one: `git grep -n <term>` plus `git ls-files --others --exclude-standard | xargs grep -ln <term>`. Explicitly enumerate other live contracts in `active/` and check whether the change breaks any of their pinned values or scope premises; when it does, rule on ordering (withdraw, re-pin, or sequence) inside the new contract rather than discovering it at Gate 4.
- **failure_mode**: Naive default: prescribe `git grep` across the whole repo as the thorough option, precisely to avoid hand-picked directories. Why wrong: it silently excludes untracked files, which in a TAD-style repo is exactly where in-flight contracts live — so the scan reads as exhaustive while omitting the artifacts most likely to conflict, and the conflict surfaces only when the other contract's AC turns red.
- **Grounded in**: HANDOFF-20260805-cut-routing-machinery.md §6.2 item 3, Gate 2 v2 adversarial report (untracked-scan finding), .tad/archive/handoffs

### A Change That Makes a Dormant Defect Reachable Owns That Defect — "I Didn't Touch That Line" Is Not a Defense - 2026-08-06

- **Context**: Two consecutive tickets in one Epic; the same reviewer corrected the same phrasing twice. **First**: the contract's risk section said of four known gaps "these existed before this ticket and are not introduced by it," and deferred them. The reviewer replied that the correct test is **reachability**, then checked each gap against the post-change rule set (all four turned out unreachable, so the deferral stood — but on the reviewer's grounds, not the author's). **Second**, one ticket later: a line in another protocol section read "boundary: do not write the full session-state.md." The new ticket granted exactly that write permission. The author wrote "this is a pre-existing wording issue, not introduced here." The reviewer: the line is indeed unchanged, **but the salience of the juxtaposition is created by this ticket** — before it, the file forbade that write outright, so no reader ever had to reconcile "you may write it" with "the boundary is not writing it."
- **Discovery**: Both times the move was substituting **authorship** ("I did not write that line") for **causation** ("I made that line start mattering"). The substitution resists self-inspection because its first clause is true, and it reads like a finished argument. Note the escalation in subtlety: the first instance concerned a *defect becoming reachable*; the second concerned only *two sentences now sitting side by side* — not a defect at all, merely an ambiguity a reader must now resolve. The cheap correct response was never "fix it" (that would breach the ticket's own no-touch scope promise) but "record it" — the contract already had a section whose stated discipline was *log known exposure honestly, do not pretend it is solved*, and the item simply had not been routed there.
- **Action**: Whenever you are about to write "pre-existing / not introduced by this ticket," answer two questions first: (a) does this change alter the defect's **reachability**? (b) does it increase what a reader must **actively disambiguate**? If either is yes, it falls inside this ticket's responsibility surface — not necessarily to fix (scope promises are binding), but to record in the risk section, which costs one line. Never let authorship close the question.
- **failure_mode**: Naive default: on finding a flaw, confirm "this line isn't mine," classify it as pre-existing, and defer it to a later phase. Why wrong: authorship and causal responsibility are different things — a dormant flaw woken by your change (now reachable, now salient, now requiring disambiguation) transfers to you, and because "I didn't touch that line" is factually true it never trips a self-check.
- **Grounded in**: HANDOFF-20260806-lite-takes-over-full.md §6 (reachability table, rewritten on Gate 2 security-auditor's grounds), HANDOFF-20260806-lite-full-parity.md §6 (Lite Progress juxtaposition entry, added on Gate 2 config-manager P2-1)/withdrawn/HANDOFF-20260804-full-reviewer-tier-rule.md AC8(b)

### Before Relaxing an Over-Tight Constraint, Inventory What It Was Guaranteeing for Free - 2026-08-11

- **Context**: A publish mandate bound `local_commit` to "exactly two append-only commits" and pinned the push/tag target to a specific SHA. At the pre-push gate two ACs failed, and both repairs landed on that commit's *content* — leaving no legal channel: amend forbidden, an extra commit would fail the `== 2` assertion, and the tag was pinned to the defective commit anyway. The count was correctly identified as a mis-specification (an established principle already says commit cardinality is agent-owned technical mechanics, not a human authorization field) and was removed. **Removing it silently destroyed a second, unwritten property**: while the target was a pinned SHA, "the artifact that was reviewed" and "the artifact that gets published" were the same object *by construction*. With the target floating to "the recorded tip", an implementer resolving that as `HEAD` at push time would publish anything committed after the review — and the same contract manufactured exactly such a candidate, since it required the pre-push review verdict to be written back into the contract file, which is inside the authorized pathspec.
- **Discovery**: An over-tight constraint is rarely load-bearing for only the reason it was written. Because it is tight, it *incidentally* pins other properties that nobody ever had to state — here, target identity stability across a time window. Those incidental guarantees are invisible precisely because they were never written down, so removing the constraint removes them silently and the contract still reads complete. The tell is that the constraint sat upstream of something else in the same causal chain: if `X == literal K` and a later step consumes `X`, then relaxing `X` changes that step's semantics even though the step's own text is untouched.
- **Action**: Before removing or widening any constraint in an accepted contract, enumerate every property that currently holds *because* of it — not just the one it was written for — and re-establish each explicitly in the same edit. Concretely: grep for every downstream reference to the constrained value and ask "does this still mean the same thing if the value can now vary?" For identity-of-target cases, the replacement is an explicit equality assertion against a recorded value plus a prohibition on re-resolving the target symbolically (`HEAD`, branch names) at use time.
- **failure_mode**: Naive default: the constraint is demonstrably wrong (it blocks legitimate work and contradicts an established principle), so delete it and move on. Why wrong: the deletion is correct on its own terms while silently withdrawing an unstated guarantee that downstream steps were relying on — and the contract passes its own review, because nothing in it ever asserted the property that just disappeared.
- **Grounded in**: `.tad/archive/handoffs/LITE-20260811-1512-publish-v2410.md` rev5 `local_commit` amendment (mandate revision 3) and the rev6 D-2 repair (AC9pre clause iii + AC10a, literal-SHA requirement); `patterns/gate-design.md` "Human Blast Radius Is an Effect Boundary, Not a Technical Counter" (2026-08-10), the principle the removal correctly applied

### Retargeting a Repeated Reference: the Instance You Miss Is the One Carrying Authority - 2026-08-11

- **Context**: The same contract changed its push/tag target from a pinned commit to "the recorded tip". Six references were updated across the decisions section and the acceptance criteria. The seventh — inside the Execution Mandate's `consequence_bindings` — was missed, so the accepted permission carrier still read `仅 git push origin <commit B>:refs/heads/main`. That commit was the one holding the *broken* verifier the gate had just rejected. The mandate therefore authorized publishing exactly the defective artifact and, via the word 仅, forbade publishing the repaired one — inside the one document the governing skill instructs the executor to re-read before every mutation.
- **Discovery**: Repeated references cluster by document section, and the author sweeps the section they are actively editing — decisions, ACs, prose. The **authority-carrying** instance almost always lives in a *different* section (the permission block, the mandate, the config schema) because that is a different kind of statement about the same value. So a sweep that feels thorough — "I changed it everywhere I was working" — systematically misses the one instance where being wrong is not a documentation defect but a granted permission. The asymmetry is the point: narrative instances fail loudly on next read; the authority instance fails silently and is obeyed.
- **Action**: After any rename or retarget, grep the **whole** document (not the section) for the old token and classify every hit as narrative/historical versus executable/authoritative. Verify the authoritative ones by name and out loud — permission bindings, schema fields, command templates — and only then declare the sweep done. A grep returning zero hits in the authority section is a result to state explicitly, not to assume.
- **failure_mode**: Naive default: update the references you encounter while making the change, then re-read the sections you edited to confirm. Why wrong: that confirms exactly the region you already looked at, while the permission carrier sits in a section you had no reason to open — and it is the only instance whose error grants rather than merely misinforms.
- **Grounded in**: `.tad/archive/handoffs/LITE-20260811-1512-publish-v2410.md` rev6 D-1 repair (both `remote_ref_update` and `remote_tag_create` bindings); the rejected tip `8b0c40a` verified to contain the defective `verify.sh` (`$EVDIR` intermediates at its `:150` and `:213`); `.claude/skills/release-runbook/SKILL.md` ("On resume and before every mutation, re-read the accepted mandate revision, exact consequence/target binding…")

### In a Capability-Retirement Inventory, "Not Needed" Is a Judgment, Not a Carrier — the Loss Concentrates in the Rows Exempted From Naming One - 2026-08-12

- **Context**: An Epic retiring a heavyweight agent channel opened with an inventory of 19 capabilities, each assigned a disposition and, for those judged already-covered, a named `existing_equivalents` plus `carrier_paths`. Five phases in, the human asked the question the inventory had not: after the migration, has the capability actually transferred *without loss*? A mechanical audit followed — for each row, name the file, hook, or skill that would still do the work if the old channel were frozen today. All ten named carriers existed and were independently invocable, so the rows everyone worried about were fine. The single confirmed loss sat in a row dispositioned RETIRE, whose rationale read "a fixed per-session startup cost, of no value in the lite-first world" and whose equivalents list was empty by construction. What it retired was a bundle of startup scans, one of which checked whether the dependency registry had gone stale.
- **Discovery**: An inventory format that requires a carrier for "covered" and requires nothing for "not needed" is not uniformly rigorous — it is rigorous exactly where the risk isn't. A covered row must name something, so it can be checked, and checking found no problems. A not-needed row asserts an absence, so there is nothing to check and no later step that can fail; a wrong judgment there is undetectable by construction and stays wrong silently. The bundling makes it worse: the retired row named a *mechanism* (startup routing and its scans) rather than the *functions* riding on it, and the rationale priced the mechanism as overhead — true of the routing, false of the scans, and the bundle let one judgment cover both. Two properties reliably mark this shape: the disposition is justified by cost rather than by redundancy, and it names a delivery mechanism rather than an outcome. The audit question that exposes it is not "was this dispositioned" — that re-runs the original judgment — but "if the old channel were frozen today, what would stop happening, and what carries it now"; that one is falsifiable, and a row that cannot answer it is a loss regardless of what its rationale says.
- **Action**: When retiring a channel, run a loss audit before freezing anything: for every row, including and especially those marked not-needed, name a concrete carrier or record a loss. Refuse cost-only rationales — "fixed overhead", "no longer worth it" — until the functions riding on the mechanism are enumerated separately and dispositioned individually. Distrust any row that names a mechanism where the others name outcomes. Note that platform-level carriers (session hooks, CI) survive channel retirement while agent-body content does not, which makes them the cheap landing spot for functions that turn out to be worth keeping.
- **failure_mode**: Naive default: audit the rows marked "already covered" — they are the ones with equivalents to verify — and accept the "not needed" rows, since a retirement decision has nothing to check. Why wrong: the covered rows were forced to name a carrier and therefore hold up under inspection; the not-needed rows were exempted from naming anything, so an over-broad retirement leaves no artifact that can fail, and the function simply stops happening with no mechanism left to notice.
- **Grounded in**: `.tad/evidence/designs/full-capability-extraction/capability-disposition.yaml`（`full-router-startup`: disposition RETIRE, `existing_equivalents: []`, rationale "每 session 固定启动费在 lite-first 下无价值"，summary 里同时打包"依赖演进/研究图景/僵尸 handoff 提示"）；2026-08-12 实测 `grep -rl 'scan-results\|last_scan' .tad/hooks/*.sh` 无结果（无 hook 载体）；`.tad/dependencies/REGISTRY.yaml` `last_checked` 停在 2026-07-14 达 28 天；`.tad/evidence/acceptance-tests/dependency-ops-skill/AC9.txt`（该 28 天窗口内 `gh` 2.97.0 修复了 4 个安全漏洞，含 `gh auth status` 明文打印部分 token）；EPIC-20260809 「Phase 5 重定义 —— 能力损耗审计」节

### Change Size Does Not Predict Verification Cost — Sizing the Process by Files-Touched Is a Measured Counterexample - 2026-08-12
- **Context**: Phase 1b 立项时按 Adaptive Complexity 评为 **small / Light TAD**，理由充分且当时看来正确：单一产物文件、15 行与 14 个载体全部复用上一 phase 的成果、不新增枚举、不重判任何一行、不触碰任何协议契约——**"是增补不是重做"**。实际走完设计期：4 份并行审查、**21 个 P0**、3 次 rev、785 行契约。
- **Discovery**: 「文件数少 / 是增补 / 不改协议 / 不重判」这几个**改动规模**信号，与**验证成本**没有可靠相关性。本单的成本几乎全部来自一个与改动规模无关的性质：**产物是判据本身**——要新增 6 个验证子命令、要给每条 AC 设计负控、要证明判据真的在读它声称在读的源。一次改 50 个文件但判据形态不变的重构，验证成本可能远低于此。反过来说：**真正的成本驱动是"这次要新建多少个判断"**（本单两个新判断列，21 个 P0 里大半集中在它们和它们的验证上），不是"这次要改多少行"。
...
### 立项理由若是「某文档在误导用户」，先去找真正在跑的那段代码 - 2026-08-14

- **Context**: 路由脱钩单的立项第一句是「`INSTALLATION_GUIDE.md` 仍在告诉每个新装的项目
  默认走 lite」，据此圈定 7 个文件。
- **Discovery**: 独立穷举审查实测为 **15 个文件**。最严重的遗漏是 **`tad.sh` 第 1514/1515/1887 行**
  ——**安装脚本本身**，每次安装真正执行、并逐字打印 `Default channel` 指向要废弃的通道。
  即：立项理由说"安装说明在误导用户"，而改动集只覆盖了那份**说明**，漏掉了**执行体**。
  同类遗漏还有 `/tad-help`（用户问"我该用哪个"时的产品内答案）。词表也失效了——
  穷举扫描的关键词是从已找到的那 14 行反推的，而执行体用的是另一种说法
  （英文、首字母大写的 `Default channel`），**用改动集推出的词表永远发现不了改动集之外的表述**。
- **Action**: 当一单的立项理由形如「X 在告诉用户错误的事」时，动手前先问两句：
  **(1) 用户实际是从哪里看到这句话的？(2) 那个位置是文档，还是一段会执行的代码？**
  文档与执行体几乎总是成对存在（安装说明 ↔ 安装脚本、帮助文档 ↔ help 命令、
  README ↔ 首屏输出），而只有后者会在用户不阅读时依然生效。穷举扫描的词表**必须独立采样**
  （从全仓真实语料里搜同义表述），不得从已知改动集反推。
- **failure_mode**: 朴素默认：立项理由指向哪个文件就改哪个文件，再顺着它的措辞去搜同类。
  为什么错：措辞是从同一份文档里抄的，因此搜到的永远是同一类载体；
  真正在运行的那段代码往往用另一种语言、另一种大小写、另一个词，从而完整地躲过整轮清理，
  而它恰恰是唯一在用户不阅读时仍然生效的那个。
- **Grounded in**: `.tad/archive/handoffs/HANDOFF-20260814-routing-decouple.md` §1（rev1→rev2 更正）；
  Gate 2 穷举审查（15 文件 vs rev1 的 7）；`tad.sh:1514,1515,1887`

### 枚举的完整性由语料清单封顶，而语料清单通常是隐式的 - 2026-08-14

- **Context**: TAD 用两轮 Epic 盘出一份「质量纪律清单」共 15 条，此后被当作全集，
  下游据它判断"哪些纪律可以改成按需加载"。
- **Discovery**: 该清单的枚举语料只有 3 个来源，且**这个事实只写在一份过程记录里、从未进入结论**。
  从未扫过 `gate/SKILL.md`（995 行，全仓强制条款密度最高）与激活时强制加载的 6 个 config。
  补扫后 **15 → 30 条**，即原枚举**只抓到一半**；漏掉的含"研究先行""技术决策透明"
  "禁止代答"这类每单必做项。更隐蔽的是：**下游那张表的验收标准要求产物与 15 条逐字相等**
  ——本意防漂移，实际是把不完整集合**盖章成全集并全绿**。
- **Action**: 任何声称"全集"的枚举，必须把**语料清单本身作为数据冻结**（路径列表，不是散文），
  并按可枚举单元（协议块/文件/条目）产出**逐单元归宿表**：每个单元要么映射到已有条目、
  要么新增、要么标"非此类 + 理由码"。**"我找到了 N 条"不是穷举，"语料里每个单元都有归宿"才是。**
  下游引用该清单时，先问一句：**它的语料清单在哪、是不是数据？** 不是数据就当它不完整。
- **failure_mode**: 朴素默认：把上一轮认真做过的枚举结果当作全集，因为它有过程记录、有评审、
  看起来严谨。为什么错：严谨性只覆盖"在语料内找得准"，覆盖不了"语料选得全"；
  而语料范围往往是当时随手划的、写在过程文档里，结论里不体现——于是每个下游都在一个
  沉默的边界内工作，且**越是用"必须与既有逐字相等"来防漂移，越是把这个边界焊死**。
- **Grounded in**: `.tad/archive/handoffs/COMPLETION-20260815-discipline-enumeration.md`；
  `disposition-60.md`（60 块：26 非纪律 / 19 已有 / 15 新增）；
  `HANDOFF-20260815-discipline-enumeration.md` §3 的语料表与具名延期

### 一张单里放"要改的东西"和"验证它的机器"，AC 写多少版都解不开 - 2026-08-15
- **Context**: 英文化单的 S0 要往 `gate/SKILL.md` 插一个检测锚。而地板表
  （`discipline-floor.md`，由 `gen-floor.py` 生成）记录着一行"载体文件字节总和"，
  `gate/SKILL.md` 正是那 9 个载体之一。插锚 → 字节数变 → 地板表必须重生成。
  **两条路都坏**：把生成器做成干跑不写文件 → 地板表**静默过时**、没有任何 AC 会红；
  照现状写文件 → 地板表变脏而它不在写权限里 → **围栏判红**。
- **Discovery**: 这不是 AC 措辞问题。三轮专家审查、18 个 P0，每轮都找出新的一批——
  因为**每加一道护栏，那道护栏都落在这一单自己要动的东西上**。
  判据：**当一张单要修改的文件集，与验证这张单的机器所读取的文件集相交时，
  这张单在结构上无法自洽验收**，无论 AC 写得多细。
- **Action**: 动手前先算这个交集。非空 → **拆单**，让每张单只碰一个文件，
  验证器与被验证物落在不同的改动里。本例的三项（Gate 3 补 `else` / SAFETY 改行集 /
  生成器加干跑）各自独立有价值、各自十几行，**捆在一起才产生死结**。
- **failure_mode**: 朴素默认：发现三个相关缺陷，合成一张单一起修，觉得"反正都是同一块地方"。
  为什么错："同一块地方"恰恰意味着它们共享验证机器——合并不是省事，
  是把三个可独立验收的改动变成一个不可验收的改动。
- **Grounded in**: `.tad/archive/handoffs/blocked/HANDOFF-20260815-english-unification.md`
  （rev1-3 与两份复审报告）；`gen-floor.py` 的载体字节总和行；`pre-gate-check.sh:191-203`
