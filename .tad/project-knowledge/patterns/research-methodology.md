...
> AMENDED 2026-09-15 (2.44.6): "NotebookLM as fallback" statements below are historical; the current research chain is `local_wiki → websearch` (SSOT: `.tad/config-workflow.yaml` → `fallback_chains.research`).
...
- **Discovery**: (1) Constitution `research/CLAUDE.md` (≤80 lines) declares unidirectional flow `raw → canon → wiki`, `generate.py` sole writer of indexes, and Iron Rule `every wiki claim needs raw_refs + locator`. (2) Controlled vocab `canon/_topics.yaml` (core 12 + allow_extend with added_by/reason) + `_questions.yaml` (6 socratic dims) gates topic sprawl; lint WARN + `⚠️ unregistered` tag preserves high-tolerance without FAIL. (3) Canon 12-field (title/what/type/citable/explores/topics/availability/verified_on/depth/wiki_page/raw_refs/provenance) body ≤15 lines/≤120 words; `citable:false` until wiki exists. (4) `ingest.sh` wraps `.tad/cross-model/source-preprocessor.sh` (detect|validate|dispatch) without reimplementing `normalize_url`/`validate_url`; yq `strenv` escapes `original_url`; `arxiv_pdf` passthrough; `x_article/bilibili/arxiv_abs/substack/medium/generic_web` supported. (5) `lint.sh` 6 rules enforce Iron Rule (YAML valid, claim==raw_refs or `[^raw/`, path exists, locator `p.|para|timestamp`, depth derived, YAML injection via `ruby -ryaml`). (6) `generate.py --emit all` is pure-function idempotent (`diff <(run1) <(run2)==0`) producing `canon/_index.md` + `wiki/index.md` + `topics/_clusters.md` from `_clusters.yaml` (optional). (7) `*research` routing now `local_wiki` primary (fallback NotebookLM/WebSearch); `research-github notebook` becomes canon shim (`*research --standard` hint). (8) Two-canon seed (`mcp-prompt-injection` + `guardrail-layers`) reuses one raw (`papers/mcp-001.md`) to prove cross-topic reuse; second query hits reuse in `_index`.
- **Action**: Keep entry discipline in skill body (principles.md:103). Never hand-write what source-preprocessor already does. Enforce saturation via wiki/log.md append-only and lint stable + zero new locators for 2 asks. Use `generate.py` for indexes, never hand-edit.
- **Grounded in**: `research/CLAUDE.md`, `research/canon/_topics.yaml`, `research/canon/_questions.yaml`, `research/canon/README.md`, `research/scripts/ingest.sh`, `research/canon/lint.sh`, `research/scripts/generate.py`, `HANDOFF-20260828-local-wiki-research-framework.md` AC-A…AC-J
- **failure_mode**: Naive default A: hand-write URL normalization/YAML escaping → injection/slug collisions. Naive default B: treat wiki as free-form prose without raw_refs → citation theater (claims without carriers). Naive default C: hand-edit `_index.md` → drift and merge conflicts; generate.py must remain sole writer.
