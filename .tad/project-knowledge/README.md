...
## Knowledge Promotion Decision Rule

When recording a learning, decide where it lives.

### → Capability Pack (`.claude/skills/{name}/SKILL.md`)

Place a learning here only if **either** condition holds:

- **≥ 2 不同项目的独立证据** (cross-project evidence) — pattern observed in 2+ different consumer projects (T3 promotion via `*harvest`).
- **行业 standard import** (industry standard reference) — verbatim lift from established public source with license verification.

### → T2 Skill-Library (`.tad/skill-library/{project}--{slug}.md`)

Place here when a learning has single-project evidence but is potentially reusable. It stays on the shelf until a second project corroborates it, then promotes to T3 (Capability Pack).
...
Phase 4 of EPIC-20260424 (2026-04-25) caught 3 over-fit items where single-project evidence was prematurely promoted to a pack. Result: Pack signals became diluted with project-specific advice. The rule keeps pack-level learning portable across consumer projects.
