...
| # | 判据 | 度量方式 |
|---|---|---|
| SC1 | 单位任务固定读取量回到出生量级 | 协议 + 强制外部文件 ≤ **20K 字符**（现 ≈74K） |
...
| 4 | 实测 + 领地复核 | ⬚ **Ready** | 度量（只读为主） | SC1/SC2 实测；用真实任务打三块保留领地，确认哪些真接不住 |
| 5 | 补齐真缺口 | ⬚ Blocked(P4) | 加法（范围由 P4 决定） | 只补 P4 证明接不住的 |
...
#### Scope
**由 P4-AC2 决定，此处不预设。** 每一项补齐必须过 P1 的定价闸。

#### Acceptance Criteria
- AC1 每项新增约束在台账中有完整三项（成本 / 挡什么 / 载体）
- AC2 补齐后固定读取量仍满足 SC1（≤20K）——**补齐不得以膨胀为代价**
- AC3 补完后升级清单对应行被删除（补一条删一行）
