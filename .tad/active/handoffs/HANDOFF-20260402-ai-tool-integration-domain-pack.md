# Handoff Template: Create New Domain Pack — AI Tool Integration
...
为 **AI Tool Integration** 创建一个 Domain Pack — 一个 YAML 配置文件，让 TAD 能在这个领域"做事"。
...
执行这 5 条 WebSearch（把 ai-tool-integration 替换为具体领域）：

```
1. "GitHub claude skills ai-tool-integration SKILL.md"
2. "GitHub AI agent ai-tool-integration skill prompt framework"
3. "GitHub awesome claude skills ai-tool-integration"
4. "ai-tool-integration best practices checklist framework"
5. "site:github.com ai-tool-integration SKILL.md OR CLAUDE.md"
```
...
**写入**：`.tad/spike-v3/domain-pack-tools/ai-tool-integration-skills-best-practices.md`
...
Agent({
  ...
  读取 .tad/domains/ai-tool-integration.yaml 的 {capability} 部分，按 steps 逐步执行。
...
| # | 文件 | 检查 |
|---|------|------|
| 1 | `.tad/spike-v3/domain-pack-tools/ai-tool-integration-skills-best-practices.md` | ≥3 仓库 × 5 维度 |
| 2 | `.tad/spike-v3/domain-pack-tools/ai-tool-integration-tool-research.md` | ≥2 工具实测 |
| 3 | `.tad/domains/tools-registry.yaml` 更新 | 新工具有完整条目 |
| 4 | `.tad/domains/ai-tool-integration.yaml` | 四层 steps + 质量标准 |
...
## Alex 的领域补充：AI Tool Integration 特定指引

### 范围定义

**Tool Integration Pack 做 agent 工具的开发和集成。**
- ✅ MCP Server 开发（为 agent 创建新工具）
- ✅ CLI 工具封装（把现有 CLI 变成 agent 可用的工具）
- ✅ API 集成（外部 API → agent tool）
- ✅ Tool schema 设计（输入/输出/错误定义）
- ✅ 工具权限设计（read-only / destructive / concurrent 标注）
- ✅ 工具测试（mock + 集成测试）
- ✅ 工具文档（作为合约的文档标准）
- ❌ Agent 架构设计（ai-agent-architecture pack）
- ❌ Prompt 设计（ai-prompt-engineering pack）
...
| Capability | 类型 | 做什么 |
|-----------|------|--------|
| `mcp_server_development` | Code B | 开发新的 MCP server |
| `cli_tool_wrapping` | Code B | 把 CLI 工具封装为 agent 可用的格式 |
| `api_integration` | Code B | 外部 API → agent tool |
| `tool_schema_design` | Doc A | 输入/输出 schema + 错误码 + 权限声明 |
| `tool_permission_model` | Doc A | read-only / destructive / concurrent 分类 |
| `tool_testing` | Code B | Mock + 集成测试 + 边界测试 |
| `tool_documentation` | Doc A | 工具文档即合约（purpose + example + strict types） |

### Phase 1 搜索关键词
```
"GitHub MCP server development tutorial SKILL.md"
"GitHub claude skills MCP tool creation"
"MCP server SDK documentation 2026"
"AI agent tool design best practices"
"function calling tool schema design patterns"
"Claude Code tool system architecture"
```

### 测试议题
**"为 Menu Snap 创建一个菜品数据库 MCP server"** — 连接真实数据源
