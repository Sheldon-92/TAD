# Handoff Template: Create New Domain Pack — AI Agent Architecture
...
为 **AI Agent Architecture** 创建一个 Domain Pack — 一个 YAML 配置文件，让 TAD 能在这个领域"做事"。
...
执行这 5 条 WebSearch（把 ai-agent-architecture 替换为具体领域）：

```
1. "GitHub claude skills ai-agent-architecture SKILL.md"
2. "GitHub AI agent ai-agent-architecture skill prompt framework"
3. "GitHub awesome claude skills ai-agent-architecture"
4. "ai-agent-architecture best practices checklist framework"
5. "site:github.com ai-agent-architecture SKILL.md OR CLAUDE.md"
```
...
**写入**：`.tad/spike-v3/domain-pack-tools/ai-agent-architecture-skills-best-practices.md`
...
Agent({
  ...
  读取 .tad/domains/ai-agent-architecture.yaml 的 {capability} 部分，按 steps 逐步执行。
...
| # | 文件 | 检查 |
|---|------|------|
| 1 | `.tad/spike-v3/domain-pack-tools/ai-agent-architecture-skills-best-practices.md` | ≥3 仓库 × 5 维度 |
| 2 | `.tad/spike-v3/domain-pack-tools/ai-agent-architecture-tool-research.md` | ≥2 工具实测 |
| 3 | `.tad/domains/tools-registry.yaml` 更新 | 新工具有完整条目 |
| 4 | `.tad/domains/ai-agent-architecture.yaml` | 四层 steps + 质量标准 |
...
## ⚠️ 这个 Pack 和其他 Pack 不同

其他 Domain Pack（web-frontend, web-backend 等）教"用什么工具做什么事"。
这个 Pack 教"**怎么在任何环境中构建可靠的 agent**"。

不绑定特定框架（不只是 Claude Code，也适用于 OpenClaw、LangChain、自建）。

---

## Alex 的领域补充：AI Agent Architecture 特定指引

### ⚠️ 必读研究材料（Alex 已完成深度研究）

**在开始 Phase 1 之前，先读完这个文件：**

📖 **`.tad/spike-v3/domain-pack-tools/ai-agent-architecture-research.md`**

这是 Alex 跨 4 个来源（Claude Code 源码 + OpenClaw 项目 + Anthropic API + 主流框架）的综合研究。包含：
- 三层可靠性模型（Prompt → Hook → 架构约束）
- 不同环境的 enforcement 机制对比表
- OpenClaw 的现有架构分析（AGENTS.md/SOUL.md/HEARTBEAT.md）
- 8 个通用 agent 架构模式
- 关键数字："10步85%准确率→80%失败率"

**你的 Phase 1 研究是在这个基础上扩展，不是从零开始。**
...
**区别**: Claude Code = "应该怎么做"。OpenClaw = "用户现在怎么做"。Pack 帮用户从后者演进到前者。

### 范围定义

**这个 Pack 做 agent 架构设计 — 从概念到可实现的蓝图。产出是设计文档 + 架构图，不是代码。**

- ✅ Agent 可靠性设计（三层验证、enforcement 机制选型）
- ✅ 角色与行为设计（persona、职责、禁止行为、行为边界）
- ✅ 工具系统设计（工具清单、权限模型、并发策略、schema 合约）
- ✅ 记忆系统设计（会话内/跨会话/持久化、检索策略）
- ✅ 多 Agent 协作（分工、通信协议、隔离机制）
- ✅ 安全约束设计（防越权、防幻觉、防无限循环、人在回路边界）
- ✅ System Prompt 架构（分层组装、缓存策略、context budget）
- ✅ 生产化设计（状态持久化、错误恢复、可观测性、降级策略）
- ❌ Prompt 具体编写（ai-prompt-engineering pack）
- ❌ MCP/CLI 工具实现（ai-tool-integration pack）
- ❌ Agent 行为评估（ai-evaluation pack）

### 建议的 Capabilities（基于 8 个通用模式）

| Capability | 类型 | 做什么 | 关键产出 |
|-----------|------|--------|---------|
| `reliability_design` | Doc A | 三层验证设计（schema→语义→权限），enforcement 机制选型 | 验证架构图 + 机制选型文档 |
| `role_behavior_design` | Doc A | Agent 角色定义、行为边界、不可变约束 | 角色文档 + 行为矩阵 + 禁止清单 |
| `tool_system_design` | Doc A | 工具清单、权限模型、并发策略、schema 合约 | 工具架构图 + 权限矩阵 |
| `memory_design` | Doc A | 记忆架构（短期/长期/持久）、检索策略、context budget | 记忆架构图 + 存储方案 |
| `multi_agent_design` | Doc A | 协作模式、通信协议、隔离机制 | 协作流程图 + 通信规范 |
| `safety_design` | Doc A | 防越权、防幻觉、circuit breaker、人在回路边界 | 安全约束清单 + 执行机制 |
| `prompt_architecture` | Doc A | System prompt 分层、组装流程、缓存策略 | Prompt 架构图 + 组装规范 |
| `production_readiness` | Doc A | 状态持久化、错误恢复、可观测性、降级、检查清单 | 生产化检查清单 + 运维文档 |

### Phase 1 搜索关键词

除了模板的通用搜索词，加：
```
"GitHub AI agent architecture design patterns SKILL.md"
"GitHub LLM agent reliability guardrails"
"AI agent safety constraints design 2026"
"multi-agent orchestration patterns production"
"LLM agent memory system design comparison"
"AI agent tool use design permission model"
"system prompt engineering architecture layers"
"agent reliability production patterns error handling"
"NeMo Guardrails vs custom guardrails comparison"
"Claude Code agent architecture source analysis"
```

**但 Phase 1 的核心不是搜 GitHub** — 是读 Alex 的研究文档 + Claude Code 源码 + OpenClaw 项目。GitHub 搜索是补充，不是主体。
...
- D2（架构图、流程图、状态机）✅
- Typst（PDF 设计文档）✅
- Mermaid.ink API（序列图）— 可能需要

可能需要新增的：
- Anthropic SDK（测试 tool_use schema）— 需要评估
- Agent 评估工具 — 待研究（可能没有好的 CLI 工具）

### 测试议题

**"为 Menu Snap 设计一个 AI 菜品推荐 agent"**：
- **角色**: 菜品推荐助手（分析菜单照片 → 推荐菜品 → 记住偏好）
- **工具**: 图片识别 API + 菜品数据库 + 用户偏好存储
- **记忆**: 过敏信息、口味偏好、历史点餐（跨会话持久化）
- **安全**: 不给医疗建议、不推荐已知过敏食物、不确定时标注 [UNVERIFIED]
- **多 agent**: 可能 — 识别 agent + 推荐 agent + 安全检查 agent
- **可靠性**: tool_use strict schema、过敏检查是架构约束（不是 prompt 建议）

这个议题能测试全部 8 个 capability，因为它有：
- 真实的安全需求（过敏 = 不可逆风险）
- 多工具调用（图片 API + 数据库 + 存储）
- 记忆需求（跨会话偏好）
- 可能的多 agent 协作
