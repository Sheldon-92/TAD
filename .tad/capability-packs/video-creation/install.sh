#!/usr/bin/env bash
# Video Creation Capability Pack installer
# Copies pack files to the target AI agent's skills directory

set -euo pipefail

PACK_NAME="video-creation"
PACK_VERSION="0.1.0"
...
check_prerequisites() {
  ...
  if command -v node >/dev/null 2>&1; then
    ...
    if [[ "$NODE_MAJOR" -ge 22 ]]; then
      ...
      warnings+=("Node.js ≥22 required (found v${NODE_VERSION})")
      echo "⚠️   Node.js: v${NODE_VERSION} (requires ≥22)"
      echo "    Upgrade: nvm install 22  OR  brew install node@22"
    fi
  else
    missing+=("Node.js ≥22")
    echo "❌  Node.js: not found"
    echo "    Install: nvm install 22  OR  brew install node@22"
  fi

  # HyperFrames
  if npx hyperframes --version >/dev/null 2>&1; then
    HF_VERSION=$(npx hyperframes --version 2>/dev/null || echo "unknown")
    echo "✅  HyperFrames: ${HF_VERSION}"
  else
    echo "ℹ️   HyperFrames: not found (optional — required for HTML-native video)"
    echo "    Install: npm install -g hyperframes  OR use via npx"
  fi

  # Remotion
  if npx remotion --version >/dev/null 2>&1; then
    echo "✅  Remotion: $(npx remotion --version 2>/dev/null || echo "found")"
  else
    echo "ℹ️   Remotion: not found (optional — required for React-based video)"
    echo "    Install per project: npm install remotion  (see remotion.dev)"
  fi
...
install_cursor() {
  ...
  echo "  - Create .cursorrules entry for video context detection"
  echo ""
  echo "For now, copy files manually:"
