...
echo "=== AI Prompt Engineering Capability Pack Installer ==="
...
case "$AGENT" in
  ...
  codex)
    ...
    echo "   AGENTS.md (project root, appended with 'ai-prompt-engineering' role reference)" >&2
...
declare -a COPY_PAIRS=(
  ...
  "references/claude.md:${TARGET_DIR}/references/claude.md"
  "references/failure-catalog.md:${TARGET_DIR}/references/failure-catalog.md"
  "references/ci-cd-templates.md:${TARGET_DIR}/references/ci-cd-templates.md"
  "references/few-shot-design.md:${TARGET_DIR}/references/few-shot-design.md"
  "references/output-format.md:${TARGET_DIR}/references/output-format.md"
  "tools/selection-matrix.md:${TARGET_DIR}/tools/selection-matrix.md"
  "tools/promptfoo-starter.yaml:${TARGET_DIR}/tools/promptfoo-starter.yaml"
  "checklists/pre-deploy.md:${TARGET_DIR}/checklists/pre-deploy.md"
  "checklists/regression.md:${TARGET_DIR}/checklists/regression.md"
  "examples/system-prompt-template.md:${TARGET_DIR}/examples/system-prompt-template.md"
)
...
trap 'rm -f "${TARGET_DIR}/.write-test" 2>/dev/null' EXIT INT TERM
if ! touch "${TARGET_DIR}/.write-test" 2>/dev/null; then
  echo "Error: ${TARGET_DIR} is not writable" >&2
  exit 1
fi

mkdir -p "${TARGET_DIR}/references" "${TARGET_DIR}/tools" "${TARGET_DIR}/checklists" "${TARGET_DIR}/examples"
