#!/usr/bin/env bash
# install.sh — Code Security Capability Pack installer
...
echo "=== Code Security Capability Pack Installer ==="
...
case "$AGENT" in
  ...
  codex)
    ...
    echo "   AGENTS.md (project root, appended with 'code-security' role reference)" >&2
  ...
  cursor)
    ...
    echo "   Phase 3 will install to:" >&2
    echo "   .cursor/rules/code-security.md" >&2
    echo "   For now, install with --agent=claude-code and adapt manually." >&2
    exit 2
    ;;
  gemini)
...
TARGET_DIR="${CLAUDE_DIR}/skills/code-security"
...
declare -a COPY_PAIRS=(
  ...
  "references/sast-rules.md:${TARGET_DIR}/references/sast-rules.md"
  "references/dast-rules.md:${TARGET_DIR}/references/dast-rules.md"
  "references/secret-detection-rules.md:${TARGET_DIR}/references/secret-detection-rules.md"
  "references/iac-security-rules.md:${TARGET_DIR}/references/iac-security-rules.md"
  "references/vulnerability-triage-rules.md:${TARGET_DIR}/references/vulnerability-triage-rules.md"
)
...
echo "  Reference 'code-security' skill in your conversation."
echo "  Or: 'Use the code-security capability pack to review this security setup.'"
