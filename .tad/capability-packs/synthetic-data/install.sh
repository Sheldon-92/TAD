#!/usr/bin/env bash
# install.sh — Synthetic Data Capability Pack installer
...
echo "=== Synthetic Data Capability Pack Installer ==="
...
case "$AGENT" in
  ...
  codex)
    ...
    echo "   AGENTS.md (project root, appended with 'synthetic-data' role reference)" >&2
  ...
  cursor)
    ...
    echo "   Phase 3 will install to:" >&2
    echo "   .cursor/rules/synthetic-data.md" >&2
    echo "   For now, install with --agent=claude-code and adapt manually." >&2
    exit 2
    ;;
  gemini)
...
TARGET_DIR="${CLAUDE_DIR}/skills/synthetic-data"
...
declare -a COPY_PAIRS=(
  ...
  "references/quality-filtering-rules.md:${TARGET_DIR}/references/quality-filtering-rules.md"
  "references/deduplication-rules.md:${TARGET_DIR}/references/deduplication-rules.md"
  "references/synthetic-generation-rules.md:${TARGET_DIR}/references/synthetic-generation-rules.md"
  "references/preference-alignment-rules.md:${TARGET_DIR}/references/preference-alignment-rules.md"
  "references/contamination-detection-rules.md:${TARGET_DIR}/references/contamination-detection-rules.md"
)
...
echo "  Reference 'synthetic-data' skill in your conversation."
echo "  Or: 'Use the synthetic-data capability pack to review this dataset pipeline.'"
