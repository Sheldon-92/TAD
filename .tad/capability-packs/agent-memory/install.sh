#!/usr/bin/env bash
# install.sh — Agent Memory & Context Engineering Capability Pack installer
...
echo "=== Agent Memory & Context Engineering Capability Pack Installer ==="
...
case "$AGENT" in
  ...
  codex)
    ...
    echo "   AGENTS.md (project root, appended with 'agent-memory' role reference)" >&2
  ...
  cursor)
    ...
    echo "   Phase 3 will install to:" >&2
    echo "   .cursor/rules/agent-memory.md" >&2
    echo "   For now, install with --agent=claude-code and adapt manually." >&2
    exit 2
    ;;
  gemini)
...
TARGET_DIR="${CLAUDE_DIR}/skills/agent-memory"
...
declare -a COPY_PAIRS=(
  ...
  "references/memory-architecture.md:${TARGET_DIR}/references/memory-architecture.md"
  "references/context-compaction.md:${TARGET_DIR}/references/context-compaction.md"
  "references/memgpt-letta-mem0.md:${TARGET_DIR}/references/memgpt-letta-mem0.md"
  "references/state-persistence.md:${TARGET_DIR}/references/state-persistence.md"
  "references/prompt-caching.md:${TARGET_DIR}/references/prompt-caching.md"
)
...
echo "  Reference 'agent-memory' skill in your conversation."
echo "  Or: 'Use the agent-memory capability pack to review this memory design.'"
