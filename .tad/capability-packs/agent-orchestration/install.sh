#!/usr/bin/env bash
# install.sh — Agent Orchestration Capability Pack installer
...
echo "=== Agent Orchestration Capability Pack Installer ==="
...
case "$AGENT" in
  ...
  codex)
    ...
    echo "   AGENTS.md (project root, appended with 'agent-orchestration' role reference)" >&2
  ...
  cursor)
    ...
    echo "   Phase 3 will install to:" >&2
    echo "   .cursor/rules/agent-orchestration.md" >&2
    echo "   For now, install with --agent=claude-code and adapt manually." >&2
    exit 2
    ;;
  gemini)
...
TARGET_DIR="${CLAUDE_DIR}/skills/agent-orchestration"
...
declare -a COPY_PAIRS=(
  ...
  "references/framework-selection.md:${TARGET_DIR}/references/framework-selection.md"
  "references/orchestration-patterns.md:${TARGET_DIR}/references/orchestration-patterns.md"
  "references/durable-execution.md:${TARGET_DIR}/references/durable-execution.md"
  "references/human-in-the-loop.md:${TARGET_DIR}/references/human-in-the-loop.md"
  "references/tool-permissions.md:${TARGET_DIR}/references/tool-permissions.md"
)
...
echo "  Reference 'agent-orchestration' skill in your conversation."
echo "  Or: 'Use the agent-orchestration capability pack to design this multi-agent system.'"
