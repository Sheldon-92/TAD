#!/usr/bin/env bash
# install.sh — LLM Observability Capability Pack installer
...
echo "=== LLM Observability Capability Pack Installer ==="
...
case "$AGENT" in
  ...
  codex)
    ...
    echo "   AGENTS.md (project root, appended with 'llm-observability' role reference)" >&2
  ...
  cursor)
    ...
    echo "   Phase 3 will install to:" >&2
    echo "   .cursor/rules/llm-observability.md" >&2
    echo "   For now, install with --agent=claude-code and adapt manually." >&2
    exit 2
    ;;
  gemini)
...
TARGET_DIR="${CLAUDE_DIR}/skills/llm-observability"
...
declare -a COPY_PAIRS=(
  ...
  "references/tracing-platform-rules.md:${TARGET_DIR}/references/tracing-platform-rules.md"
  "references/cost-attribution-rules.md:${TARGET_DIR}/references/cost-attribution-rules.md"
  "references/otel-semconv-rules.md:${TARGET_DIR}/references/otel-semconv-rules.md"
  "references/latency-profiling-rules.md:${TARGET_DIR}/references/latency-profiling-rules.md"
  "references/prompt-registry-rules.md:${TARGET_DIR}/references/prompt-registry-rules.md"
  "references/online-eval-drift-rules.md:${TARGET_DIR}/references/online-eval-drift-rules.md"
)
...
echo "  Reference 'llm-observability' skill in your conversation."
echo "  Or: 'Use the llm-observability capability pack to review this monitoring setup.'"
