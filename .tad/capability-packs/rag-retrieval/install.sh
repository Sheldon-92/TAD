#!/usr/bin/env bash
# install.sh — RAG & Retrieval Engineering Capability Pack installer
...
echo "=== RAG & Retrieval Engineering Capability Pack Installer ==="
...
case "$AGENT" in
  ...
  codex)
    ...
    echo "   AGENTS.md (project root, appended with 'rag-retrieval' role reference)" >&2
  ...
  cursor)
    ...
    echo "   Phase 3 will install to:" >&2
    echo "   .cursor/rules/rag-retrieval.md" >&2
    echo "   For now, install with --agent=claude-code and adapt manually." >&2
    exit 2
    ;;
  gemini)
...
TARGET_DIR="${CLAUDE_DIR}/skills/rag-retrieval"
...
declare -a COPY_PAIRS=(
  ...
  "references/chunking-rules.md:${TARGET_DIR}/references/chunking-rules.md"
  "references/embedding-rules.md:${TARGET_DIR}/references/embedding-rules.md"
  "references/vector-database-rules.md:${TARGET_DIR}/references/vector-database-rules.md"
  "references/hybrid-rerank-rules.md:${TARGET_DIR}/references/hybrid-rerank-rules.md"
  "references/graphrag-rules.md:${TARGET_DIR}/references/graphrag-rules.md"
  "references/rag-evaluation-rules.md:${TARGET_DIR}/references/rag-evaluation-rules.md"
)
...
echo "  Reference 'rag-retrieval' skill in your conversation."
echo "  Or: 'Use the rag-retrieval capability pack to review this RAG pipeline.'"
