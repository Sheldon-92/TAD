#!/bin/bash
# verify.sh — dependency-ops-skill 围栏核心（AC16/AC17）
...
BASE=".tad/evidence/acceptance-tests/dependency-ops-skill"
REPO="$(cd "$(dirname "$0")/../../../.." && pwd -P)"
FENCE_PROBE=".tad/domains/DEPS-FENCE-PROBE.txt"

# 排除前缀（契约 AC16）：本单产物 + 授权修改 + 框架自写路径
is_excluded() {
  local p="$1"
  case "$p" in
    .claude/skills/dependency-ops/*) return 0 ;;
    .agents/skills/dependency-ops/*) return 0 ;;
    .tad/evidence/acceptance-tests/dependency-ops-skill/*) return 0 ;;
    .tad/evidence/journal/dependency-ops-skill.md) return 0 ;;
    .tad/evidence/reviews/blake/dependency-ops-skill/*) return 0 ;;
    .tad/active/handoffs/LITE-20260811-2254-dependency-ops-skill.md) return 0 ;;
    .tad/active/handoffs/LITE-20260811-2254-dependency-ops-skill.md.txn-lock/*) return 0 ;;
    .tad/archive/handoffs/LITE-20260811-2254-dependency-ops-skill.md) return 0 ;;
    .tad/dependencies/REGISTRY.yaml) return 0 ;;
    .tad/dependencies/scan-results.yaml) return 0 ;;
...
baseline_dirty_tracked() {
  grep -v '^??' "$BASE/scope-baseline.txt" | grep -E '^( M|M |MM| D|D |A |A\?)' | awk '{print $3}' | LC_ALL=C sort
}
...
fence_audit() {
  ...
  while IFS= read -r p; do
    ...
    base_sha=$(grep -E "^ M [0-9a-f]{64} ${p}$" "$BASE/scope-baseline.txt" | awk '{print $2}' | head -1)
  ...
  if command -v yq >/dev/null 2>&1; then
    yq -o=props '.' "$REPO/.tad/research-notebooks/REGISTRY.yaml" | LC_ALL=C sort > /tmp/tad-reg-c.props
    ...
    # yq 缺席 → 逐字节不变（更强断言）
    if cmp -s "$BASE/registry-baseline.yaml" "$REPO/.tad/research-notebooks/REGISTRY.yaml"; then
  ...
  if [ -z "$bi_base" ]; then
    echo "BRAIN-INDEX NO-BASELINE (skip)"
  else
