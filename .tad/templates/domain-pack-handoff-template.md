# Handoff Template: Create New Domain Pack — {DOMAIN_NAME}
...
为 **{DOMAIN_NAME}** 创建一个 Domain Pack — 一个 YAML 配置文件，让 TAD 能在这个领域"做事"。
...
执行这 5 条 WebSearch（把 {DOMAIN} 替换为具体领域）：

```
1. "GitHub claude skills {DOMAIN} SKILL.md"
2. "GitHub AI agent {DOMAIN} skill prompt framework"
3. "GitHub awesome claude skills {DOMAIN}"
4. "{DOMAIN} best practices checklist framework"
5. "site:github.com {DOMAIN} SKILL.md OR CLAUDE.md"
```
...
**写入**：`.tad/spike-v3/domain-pack-tools/{DOMAIN}-skills-best-practices.md`
...
Agent({
  ...
  读取 .tad/domains/{DOMAIN}.yaml 的 {capability} 部分，按 steps 逐步执行。
...
| # | 文件 | 检查 |
|---|------|------|
| 1 | `.tad/spike-v3/domain-pack-tools/{DOMAIN}-skills-best-practices.md` | ≥3 仓库 × 5 维度 |
| 2 | `.tad/spike-v3/domain-pack-tools/{DOMAIN}-tool-research.md` | ≥2 工具实测 |
| 3 | `.tad/domains/tools-registry.yaml` 更新 | 新工具有完整条目 |
| 4 | `.tad/domains/{DOMAIN}.yaml` | 四层 steps + 质量标准 |
