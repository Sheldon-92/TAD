# Handoff Template: Create New Domain Pack — AI Evaluation
...
为 **AI Evaluation** 创建一个 Domain Pack — 一个 YAML 配置文件，让 TAD 能在这个领域"做事"。
...
执行这 5 条 WebSearch（把 ai-evaluation 替换为具体领域）：

```
1. "GitHub claude skills ai-evaluation SKILL.md"
2. "GitHub AI agent ai-evaluation skill prompt framework"
3. "GitHub awesome claude skills ai-evaluation"
4. "ai-evaluation best practices checklist framework"
5. "site:github.com ai-evaluation SKILL.md OR CLAUDE.md"
```
...
```markdown
# ai-evaluation Skills Best Practices
...
**写入**：`.tad/spike-v3/domain-pack-tools/ai-evaluation-skills-best-practices.md`
...
Agent({
  ...
  读取 .tad/domains/ai-evaluation.yaml 的 {capability} 部分，按 steps 逐步执行。
...
| # | 文件 | 检查 |
|---|------|------|
| 1 | `.tad/spike-v3/domain-pack-tools/ai-evaluation-skills-best-practices.md` | ≥3 仓库 × 5 维度 |
| 2 | `.tad/spike-v3/domain-pack-tools/ai-evaluation-tool-research.md` | ≥2 工具实测 |
| 3 | `.tad/domains/tools-registry.yaml` 更新 | 新工具有完整条目 |
| 4 | `.tad/domains/ai-evaluation.yaml` | 四层 steps + 质量标准 |
...
## Alex 的领域补充：AI Evaluation 特定指引

### 范围定义

**AI Evaluation Pack 做 agent 行为的评估和测试。**
- ✅ Agent 行为基准测试（预定义场景 → 评分）
- ✅ A/B 测试（两个 prompt/配置 → 对比效果）
- ✅ 回归测试（改了 prompt → 验证没破坏已有行为）
- ✅ 安全测试（对抗性输入 → 验证安全约束生效）
- ✅ 质量评分框架（定义什么是"好"的 agent 输出）
- ✅ 自动化评估管线（CI 集成 → 每次改动自动评估）
- ✅ 人工评估协议（human eval 的结构化流程）
- ❌ Agent 架构设计（ai-agent-architecture pack）
- ❌ Prompt 编写（ai-prompt-engineering pack）
- ❌ 工具开发（ai-tool-integration pack）

### 和 TAD 自测 agent 的关系

我们之前记录了一个 idea（IDEA-20260401-tad-self-test-agent）— 用 Agent tool spawn sub-agent 来自动测试。这个 pack 的 `automated_eval_pipeline` capability 就是这个概念的泛化版本。

### 必读参考

- `.tad/active/ideas/IDEA-20260401-tad-self-test-agent.md` — 自测 agent 概念
- `.tad/tests/test-domain-pack.md` — 我们已有的测试脚本格式
- `.tad/spike-v3/domain-pack-tools/ai-agent-architecture-research.md` — 可靠性模式

### 建议 Capabilities

| Capability | 类型 | 做什么 |
|-----------|------|--------|
| `eval_framework_design` | Doc A | 设计评估维度和评分标准 |
| `benchmark_testing` | Code B | 预定义场景 → 自动跑 → 评分 |
| `ab_testing` | Code B | 两个配置 → 同场景 → 对比 |
| `regression_testing` | Code B | 改动后 → 验证已有行为不变 |
| `adversarial_testing` | Code B | 对抗性输入 → 验证安全约束 |
| `automated_pipeline` | Code B | CI 集成 → 每次改动自动评估 |
| `human_eval_protocol` | Mixed | 人工评估的结构化流程（4D Protocol 扩展） |

### Phase 1 搜索关键词
```
"GitHub AI agent evaluation framework SKILL.md"
"LLM evaluation benchmark testing tools 2026"
"AI agent regression testing automation"
"adversarial testing LLM agents safety"
"agent evaluation metrics quality scoring"
"langfuse agent evaluation tracing"
"promptfoo LLM testing framework"
```

### 测试议题
**"评估 TAD 的 Alex agent 在不同场景下的行为一致性"** — 自测自己
