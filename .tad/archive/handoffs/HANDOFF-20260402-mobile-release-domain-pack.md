# Handoff Template: Create New Domain Pack — Mobile Release
...
为 **Mobile Release** 创建一个 Domain Pack — 一个 YAML 配置文件，让 TAD 能在这个领域"做事"。
...
执行这 5 条 WebSearch（把 mobile-release 替换为具体领域）：

```
1. "GitHub claude skills mobile-release SKILL.md"
2. "GitHub AI agent mobile-release skill prompt framework"
3. "GitHub awesome claude skills mobile-release"
4. "mobile-release best practices checklist framework"
5. "site:github.com mobile-release SKILL.md OR CLAUDE.md"
```
...
```markdown
# mobile-release Skills Best Practices
...
**写入**：`.tad/spike-v3/domain-pack-tools/mobile-release-skills-best-practices.md`
...
Agent({
  ...
  读取 .tad/domains/mobile-release.yaml 的 {capability} 部分，按 steps 逐步执行。
...
| # | 文件 | 检查 |
|---|------|------|
| 1 | `.tad/spike-v3/domain-pack-tools/mobile-release-skills-best-practices.md` | ≥3 仓库 × 5 维度 |
| 2 | `.tad/spike-v3/domain-pack-tools/mobile-release-tool-research.md` | ≥2 工具实测 |
| 3 | `.tad/domains/tools-registry.yaml` 更新 | 新工具有完整条目 |
| 4 | `.tad/domains/mobile-release.yaml` | 四层 steps + 质量标准 |
...
## Alex 的领域补充：Mobile Release 特定指引

### 范围定义

**Mobile Release Pack 做 App Store / Play Store 发布全流程。**
- ✅ App Store 审核准备（截图、描述、隐私政策、年龄分级）
- ✅ TestFlight / Internal Testing 分发
- ✅ 元数据管理（多语言标题/描述/关键词）
- ✅ 发布自动化（fastlane / EAS Submit）
- ✅ 版本管理（语义化版本 + build number）
- ✅ 审核合规检查（Apple 审核指南常见拒审原因）
- ✅ 发布后监控（Crash 报告、评分、下载量）
- ❌ 代码开发（mobile-development pack）
- ❌ 测试（mobile-testing pack）
- ❌ UI 设计（mobile-ui-design pack）

### 这个 pack 的独特性

**发布是所有 pack 中最"流程重"的** — 不像代码可以试错，App Store 提审失败要等 1-7 天重新审核。Domain Pack 的价值在于**提审前检查清单防止常见拒审**。

### 建议的 Capabilities

| Capability | 类型 | 做什么 | 工具 |
|-----------|------|--------|------|
| `store_metadata` | Code B | 截图生成 + 标题/描述/关键词 + 多语言 | fastlane screenshots / EAS |
| `testflight_distribution` | Code B | TestFlight 构建上传 + 内部测试分发 | fastlane pilot / EAS Submit |
| `review_compliance` | Doc A | Apple 审核指南检查（常见拒审原因分析） | 搜索→分析→推导→生成 |
| `privacy_policy` | Doc A | 隐私政策 + 数据使用声明 + App Privacy Labels | Write + 模板 |
| `version_management` | Code B | 语义化版本 + build number 自动递增 | fastlane / agvtool |
| `release_automation` | Code B | CI/CD → TestFlight / App Store 自动发布 | fastlane / EAS + GitHub Actions |
| `post_release_monitoring` | Mixed | Crash 监控 + 评分追踪 + 下载量 | Sentry (已有) + App Store Connect API |

### Phase 1 搜索关键词

```
"GitHub claude skills iOS release App Store SKILL.md"
"GitHub claude skills fastlane mobile deployment"
"GitHub AI agent App Store submission automation"
"Apple App Store review guidelines checklist 2026"
"fastlane best practices iOS deployment"
"EAS Submit Expo deployment guide"
"App Store rejection common reasons"
"mobile app release checklist privacy policy"
...
| 类别 | 工具 | 说明 |
|------|------|------|
| 发布自动化 | fastlane | `fastlane deliver` / `fastlane pilot` — iOS 发布标准工具 |
| 发布自动化 | EAS Submit | `eas submit --platform ios` — Expo 生态 |
| 截图 | fastlane snapshot | 自动化多设备截图 |
| 版本 | agvtool | Apple 版本号管理 CLI |
| 隐私 | 无特定工具 | 模板生成 (Write) |
| 监控 | Sentry (已有) | Crash 监控 |
| App Store API | App Store Connect API | 元数据/审核状态查询 |

### 测试议题

**"准备 Menu Snap iOS 版的 TestFlight 发布"**：
1. 生成 App Store 元数据（标题/描述/关键词）
2. 审核合规检查（常见拒审原因扫描）
3. 隐私政策生成
4. fastlane 配置文件生成（Fastfile + Appfile）
5. 版本号管理配置

### ⚠️ E2E 测试限制

和 web-deployment 类似，很多操作需要真实账号（Apple Developer Account、App Store Connect）。E2E 测试只测**配置文件生成 + 合规检查文档**，不做真实提交。
