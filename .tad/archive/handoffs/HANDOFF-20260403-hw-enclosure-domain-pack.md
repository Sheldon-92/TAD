# Handoff Template: Create New Domain Pack — HW Enclosure
...
为 **HW Enclosure** 创建一个 Domain Pack — 一个 YAML 配置文件，让 TAD 能在这个领域"做事"。
...
执行这 5 条 WebSearch（把 hw-enclosure 替换为具体领域）：

```
1. "GitHub claude skills hw-enclosure SKILL.md"
2. "GitHub AI agent hw-enclosure skill prompt framework"
3. "GitHub awesome claude skills hw-enclosure"
4. "hw-enclosure best practices checklist framework"
5. "site:github.com hw-enclosure SKILL.md OR CLAUDE.md"
```
...
```markdown
# hw-enclosure Skills Best Practices
...
**写入**：`.tad/spike-v3/domain-pack-tools/hw-enclosure-skills-best-practices.md`
...
Agent({
  ...
  读取 .tad/domains/hw-enclosure.yaml 的 {capability} 部分，按 steps 逐步执行。
...
| # | 文件 | 检查 |
|---|------|------|
| 1 | `.tad/spike-v3/domain-pack-tools/hw-enclosure-skills-best-practices.md` | ≥3 仓库 × 5 维度 |
| 2 | `.tad/spike-v3/domain-pack-tools/hw-enclosure-tool-research.md` | ≥2 工具实测 |
| 3 | `.tad/domains/tools-registry.yaml` 更新 | 新工具有完整条目 |
| 4 | `.tad/domains/hw-enclosure.yaml` | 四层 steps + 质量标准 |
...
- ✅ 人机工程（手持尺寸、按钮位置、散热）
- ✅ 制造文件导出（STL / STEP）
- ❌ 电路设计（hw-circuit-design）
- ❌ 固件（hw-firmware）

### 建议 Capabilities
| Capability | 做什么 | 工具 |
|-----------|--------|------|
| `enclosure_design` | 参数化 3D 建模 | OpenSCAD |
| `pcb_fitting` | PCB 安装 + 连接器开口 + 公差 | OpenSCAD |
| `material_selection` | 材料选型 + 工艺 + 成本 | WebSearch |
| `assembly_design` | 装配方式 + 拆装便利性 | D2 |
| `ergonomics` | 人机工程 + 外观 + 触感 | WebSearch |
| `manufacturing_export` | STL/STEP 导出 + 打印参数 | OpenSCAD |
| `enclosure_documentation` | 尺寸图 + 装配指南 | Typst, D2 |

### 测试议题
**"Wayo 大象追踪器外壳"** — 需要容纳 ESP32-C3 + 5.65" E-ink + 电池
