...
## AC
- AC1: 由独立 reviewer 在临时目录执行 `tmp="$(mktemp -d)"; trap 'rm -rf "$tmp"' EXIT; cd "$tmp"; bash "/path/to/TAD/tad.sh" --yes; test -f .claude/skills/alex-lite/SKILL.md; test -f .agents/skills/alex-lite/SKILL.md; test -f AGENTS.md; test -f .codex/hooks.json`；所有命令成功，证明无 `--platform` 的全新安装同时落地两套入口。
- AC2: 由独立 reviewer 执行 `tmp="$(mktemp -d)"; trap 'rm -rf "$tmp"' EXIT; mkdir -p "$tmp/.tad/active" "$tmp/.claude/skills/alex-lite"; printf '2.35.0\n' > "$tmp/.tad/version.txt"; printf 'keep-me\n' > "$tmp/.tad/active/user-sentinel"; printf 'legacy\n' > "$tmp/.claude/skills/alex-lite/SKILL.md"; cd "$tmp"; bash "/path/to/TAD/tad.sh" --yes; test "$(cat .tad/active/user-sentinel)" = keep-me; test -f .agents/skills/alex-lite/SKILL.md; test -f AGENTS.md; test -f .codex/hooks.json`；所有命令成功，证明仅 Claude Code 的旧项目升级后补齐 Codex 且保留项目数据。
- AC3: 由独立 reviewer 在 TAD 仓库根目录执行 `bash "/path/to/TAD/tad.sh" --help | rg -q 'claude-code, codex, both.*Default: both'`; `node "/path/to/TAD/bin/tad-install.mjs" --help | rg -q 'claude-code, codex, both.*Default: both'`; `rg -q 'PLATFORM="both"' tad.sh`; `rg -q "argPlatform \|\| 'both'" bin/tad-install.mjs`；四项均成功，证明 shell 与 npx 安装入口的有效平台列表、帮助和默认路由一致。
- AC4: 由独立 reviewer 执行 `for platform in claude-code codex both; do tmp="$(mktemp -d)"; (cd "$tmp" && bash "/path/to/TAD/tad.sh" --platform "$platform" --yes && case "$platform" in claude-code) test -f .claude/skills/alex-lite/SKILL.md && test ! -e .agents/skills && test ! -e AGENTS.md && test ! -e .codex;; codex) test -f .agents/skills/alex-lite/SKILL.md && test ! -e .claude/skills && test -f AGENTS.md && test -f .codex/hooks.json;; both) test -f .claude/skills/alex-lite/SKILL.md && test -f .agents/skills/alex-lite/SKILL.md && test -f AGENTS.md && test -f .codex/hooks.json;; esac); status="$?"; rm -rf "$tmp"; test "$status" -eq 0 || exit "$status"; done`；命令成功退出，证明三个显式平台值均保持原有覆盖语义且 `both` 同时落地两套入口。
...
- AC6: 由独立 reviewer 执行 `tmp="$(mktemp -d)"; trap 'rm -rf "$tmp"' EXIT; mkdir -p "$tmp/.tad/active" "$tmp/.claude/skills/alex-lite"; printf '2.35.0\n' > "$tmp/.tad/version.txt"; printf 'keep-me\n' > "$tmp/.tad/active/user-sentinel"; printf 'legacy\n' > "$tmp/.claude/skills/alex-lite/SKILL.md"; cd "$tmp"; node "/path/to/TAD/bin/tad-install.mjs"; test "$(cat .tad/active/user-sentinel)" = keep-me; test -f .agents/skills/alex-lite/SKILL.md; test -f AGENTS.md; test -f .codex/hooks.json`；所有命令成功，证明 npx/Node 入口的无参数升级同样补齐 Codex 且保留项目数据。
