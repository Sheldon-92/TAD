...
- Gate 4 note: the final one-line P1 fix for `askuser-capture.sh` impact is present in the policy artifact and commit, but no separate R3 spot-check review file was created. Accepted as a process blemish, not a content blocker.
...
**Gate 4 Notes**:
- Config/agent strategy accepted as **draft-only**. No active `.codex/config.toml` or `.codex/agents/*` was created.
- Model choices (`gpt-5.5`, `gpt-5.4-mini`) and key config surfaces were re-verified against the current Codex manual fetched on 2026-06-09.
- Alex and Blake remain `keep_skill_only`; custom agents are limited to narrow reviewer/expert roles.
- `ask_user_question` matcher remains `unknown_current_behavior` and must be tested in Phase 5.
- Draft TOML parses and `web_search` is root-scoped in the final committed artifact.
- Phase 3/4/5 must carry forward the 8 P2 review items and the missing R3 review-evidence blemish.
