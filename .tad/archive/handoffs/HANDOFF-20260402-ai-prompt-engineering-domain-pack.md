# Handoff Template: Create New Domain Pack — AI Prompt Engineering
...
为 **AI Prompt Engineering** 创建一个 Domain Pack — 一个 YAML 配置文件，让 TAD 能在这个领域"做事"。
...
执行这 5 条 WebSearch（把 ai-prompt-engineering 替换为具体领域）：

```
1. "GitHub claude skills ai-prompt-engineering SKILL.md"
2. "GitHub AI agent ai-prompt-engineering skill prompt framework"
3. "GitHub awesome claude skills ai-prompt-engineering"
4. "ai-prompt-engineering best practices checklist framework"
5. "site:github.com ai-prompt-engineering SKILL.md OR CLAUDE.md"
```
...
```markdown
# ai-prompt-engineering Skills Best Practices
...
**写入**：`.tad/spike-v3/domain-pack-tools/ai-prompt-engineering-skills-best-practices.md`
...
Agent({
  ...
  读取 .tad/domains/ai-prompt-engineering.yaml 的 {capability} 部分，按 steps 逐步执行。
...
| # | 文件 | 检查 |
|---|------|------|
| 1 | `.tad/spike-v3/domain-pack-tools/ai-prompt-engineering-skills-best-practices.md` | ≥3 仓库 × 5 维度 |
| 2 | `.tad/spike-v3/domain-pack-tools/ai-prompt-engineering-tool-research.md` | ≥2 工具实测 |
| 3 | `.tad/domains/tools-registry.yaml` 更新 | 新工具有完整条目 |
| 4 | `.tad/domains/ai-prompt-engineering.yaml` | 四层 steps + 质量标准 |
...
## Alex 的领域补充：AI Prompt Engineering 特定指引

### 范围定义

**Prompt Engineering Pack 做 prompt 的设计、测试、优化、版本管理。**
- ✅ System prompt 设计（角色定义、行为约束、输出格式）
- ✅ Few-shot 示例设计（选什么示例、怎么组织）
- ✅ Prompt 测试（边界测试、对抗测试、一致性测试）
- ✅ Prompt 优化（迭代改进、A/B 对比）
- ✅ Prompt 版本管理（变更追踪、回滚）
- ✅ 输出格式控制（JSON/Markdown/结构化输出）
- ✅ Context 管理（token budget、信息优先级、压缩策略）
- ❌ Agent 架构设计（ai-agent-architecture pack）
- ❌ 工具/MCP 实现（ai-tool-integration pack）
...
| Capability | 类型 | 做什么 |
|-----------|------|--------|
| `system_prompt_design` | Doc A | 设计 system prompt（角色+约束+格式） |
| `few_shot_design` | Doc A | 选择和组织 few-shot 示例 |
| `prompt_testing` | Code B | 边界/对抗/一致性测试 |
| `prompt_optimization` | Mixed | 迭代改进 + A/B 对比 |
| `output_format_control` | Code B | JSON schema / structured output 设计 |
| `context_management` | Doc A | Token budget、信息优先级、压缩策略 |
| `prompt_versioning` | Code B | 变更追踪、diff、回滚 |

### Phase 1 搜索关键词
```
"GitHub claude skills prompt engineering SKILL.md"
"GitHub AI prompt testing evaluation framework"
"prompt engineering best practices 2026"
"system prompt design patterns LLM"
"few-shot learning prompt design"
"structured output JSON schema Claude"
"prompt version management tools"
```

### 测试议题
**"优化 Menu Snap 的菜品识别 prompt"** — 有真实的 prompt 需要优化
