# Handoff Template: Create New Domain Pack — HW Testing
...
为 **HW Testing** 创建一个 Domain Pack — 一个 YAML 配置文件，让 TAD 能在这个领域"做事"。
...
执行这 5 条 WebSearch（把 hw-testing 替换为具体领域）：

```
1. "GitHub claude skills hw-testing SKILL.md"
2. "GitHub AI agent hw-testing skill prompt framework"
3. "GitHub awesome claude skills hw-testing"
4. "hw-testing best practices checklist framework"
5. "site:github.com hw-testing SKILL.md OR CLAUDE.md"
```
...
```markdown
# hw-testing Skills Best Practices
...
**写入**：`.tad/spike-v3/domain-pack-tools/hw-testing-skills-best-practices.md`
...
Agent({
  ...
  读取 .tad/domains/hw-testing.yaml 的 {capability} 部分，按 steps 逐步执行。
...
| # | 文件 | 检查 |
|---|------|------|
| 1 | `.tad/spike-v3/domain-pack-tools/hw-testing-skills-best-practices.md` | ≥3 仓库 × 5 维度 |
| 2 | `.tad/spike-v3/domain-pack-tools/hw-testing-tool-research.md` | ≥2 工具实测 |
| 3 | `.tad/domains/tools-registry.yaml` 更新 | 新工具有完整条目 |
| 4 | `.tad/domains/hw-testing.yaml` | 四层 steps + 质量标准 |
...
| Capability | 类型 | 做什么 |
|-----------|------|--------|
| `power_on_test` | Code B | 上电检查清单 + 电压测量 |
| `functional_test` | Code B | 外设逐一验证脚本 |
| `power_measurement` | Code B | 各模式功耗 + 电池寿命计算 |
| `environmental_test` | Doc A | 环境测试方案设计 |
| `emc_precheck` | Doc A | EMC 预合规检查清单 |
| `test_fixture` | Doc A | 测试夹具设计 |
| `hw_pair_testing` | Mixed | 人+AI 协作硬件测试（4D Protocol） |

### 关键差异（vs 软件测试）
硬件测试很多**无法自动化** — 需要实际测量（万用表、示波器、功率计）。Domain Pack 的价值在于：测什么、怎么测、合格标准是什么。

### 测试议题
**"测试 Wayo 原型"** — 上电 + E-ink 刷新 + Deep Sleep 功耗 + 电池寿命
