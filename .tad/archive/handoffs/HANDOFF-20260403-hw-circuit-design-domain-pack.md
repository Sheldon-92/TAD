# Handoff Template: Create New Domain Pack — HW Circuit Design
...
为 **HW Circuit Design** 创建一个 Domain Pack — 一个 YAML 配置文件，让 TAD 能在这个领域"做事"。
...
执行这 5 条 WebSearch（把 hw-circuit-design 替换为具体领域）：

```
1. "GitHub claude skills hw-circuit-design SKILL.md"
2. "GitHub AI agent hw-circuit-design skill prompt framework"
3. "GitHub awesome claude skills hw-circuit-design"
4. "hw-circuit-design best practices checklist framework"
5. "site:github.com hw-circuit-design SKILL.md OR CLAUDE.md"
```
...
```markdown
# hw-circuit-design Skills Best Practices
...
**写入**：`.tad/spike-v3/domain-pack-tools/hw-circuit-design-skills-best-practices.md`
...
Agent({
  ...
  读取 .tad/domains/hw-circuit-design.yaml 的 {capability} 部分，按 steps 逐步执行。
...
| # | 文件 | 检查 |
|---|------|------|
| 1 | `.tad/spike-v3/domain-pack-tools/hw-circuit-design-skills-best-practices.md` | ≥3 仓库 × 5 维度 |
| 2 | `.tad/spike-v3/domain-pack-tools/hw-circuit-design-tool-research.md` | ≥2 工具实测 |
| 3 | `.tad/domains/tools-registry.yaml` 更新 | 新工具有完整条目 |
| 4 | `.tad/domains/hw-circuit-design.yaml` | 四层 steps + 质量标准 |
...
- ✅ PCB 布局（走线、层数、尺寸约束）
- ✅ BOM 管理（物料清单、供应商、成本估算）
- ✅ 电气规则检查（DRC、ERC）
- ✅ 电源设计（电池管理、稳压、功耗预算）
- ❌ 固件开发（hw-firmware pack）
- ❌ 外壳设计（hw-enclosure pack）

### 建议 Capabilities
| Capability | 做什么 | 工具 |
|-----------|--------|------|
| `component_selection` | 元器件选型 + 替代方案 + 供应链 | WebSearch, D2 |
| `schematic_design` | 原理图绘制 + 模块化设计 | KiCad CLI |
| `pcb_layout` | PCB 布局 + 走线规则 | KiCad CLI |
| `bom_management` | BOM 生成 + 成本估算 + 双供应商 | KiCad CLI, Typst |
| `power_design` | 电源方案 + 功耗预算 + 电池寿命 | D2, matplotlib |
| `design_review` | DRC/ERC + 专家审查清单 | KiCad CLI |
| `design_documentation` | 原理图文档 + 设计决策记录 | Typst, D2 |

### 用户技术栈
ESP32-S3/C3 系列、OLED (SH1106)、E-ink (Waveshare ACeP)、触觉 (DA7280)、电容触摸。arduino-cli 构建。

### 测试议题
**"Wayo 大象追踪器的电路设计"** — 用户真实项目（ESP32-C3 + 7色E-ink + RTC + 电池）
