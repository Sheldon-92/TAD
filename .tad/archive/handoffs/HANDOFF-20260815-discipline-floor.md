...
# 🚫 SUPERSEDED（2026-08-14，未交付）

**本契约从未交给 Blake。** Gate 2 三专家审查推翻了它的前提与验收设计：

1. **前提塌了**：它假设纪律共 15 条。P2a 实测补全为 **30 条**——本契约的 AC1
   （产物须与 15 条逐字相等）会把不完整集合**盖章成全集并全绿**。
2. **12 条 AC 对真正的产出（Layer 判定）零判别力**：两位审查员各自造对抗表实测——
   「15 行全判必须常驻」与「15 行全判按需」**两端都全绿通过**。
   我那条 8192 字节预算是按"全部常驻"算出来再乘 2.8 倍余量的，**结构上不可能触发**。
3. **AC12 重演了 AC9 的自证缺陷**：冻结物放在被约束方可写的目录里，
   审查员用可运行 PoC 证明"改 AC 再重冻"两条 AC 全绿。

**P2b 须重写**，可复用的部分：Layer 0/1/2 分层思路、循环触发测试作判据、
DO-178C「一套目标表 + 适用性列」的形状。**不可复用**：AC 全部，行集合基数（15→30）。
审查员给的有效修法见本文件对应 Gate 2 记录与 `patterns/gate-design.md`
「只能由被约束方执行的规则等于没有规则」。
