# Handoff Template: Create New Domain Pack — Web Deployment
...
为 **Web Deployment** 创建一个 Domain Pack — 一个 YAML 配置文件，让 TAD 能在这个领域"做事"。
...
执行这 5 条 WebSearch（把 web-deployment 替换为具体领域）：

```
1. "GitHub claude skills web-deployment SKILL.md"
2. "GitHub AI agent web-deployment skill prompt framework"
3. "GitHub awesome claude skills web-deployment"
4. "web-deployment best practices checklist framework"
5. "site:github.com web-deployment SKILL.md OR CLAUDE.md"
```
...
**写入**：`.tad/spike-v3/domain-pack-tools/web-deployment-skills-best-practices.md`
...
Agent({
  ...
  读取 .tad/domains/web-deployment.yaml 的 {capability} 部分，按 steps 逐步执行。
...
| # | 文件 | 检查 |
|---|------|------|
| 1 | `.tad/spike-v3/domain-pack-tools/web-deployment-skills-best-practices.md` | ≥3 仓库 × 5 维度 |
| 2 | `.tad/spike-v3/domain-pack-tools/web-deployment-tool-research.md` | ≥2 工具实测 |
| 3 | `.tad/domains/tools-registry.yaml` 更新 | 新工具有完整条目 |
| 4 | `.tad/domains/web-deployment.yaml` | 四层 steps + 质量标准 |
...
## Alex 的领域补充：Web Deployment 特定指引

### 范围定义

**Web Deployment Pack 做部署和运维：从代码到线上运行。**
- ✅ 部署平台选型与配置（Vercel / Netlify / Cloudflare / Docker）
- ✅ CI/CD 流水线设计（GitHub Actions / 自动化构建测试部署）
- ✅ 环境管理（开发 / Staging / 生产环境配置）
- ✅ 域名与 DNS 配置
- ✅ 监控与告警（Sentry、Uptime、性能监控）
- ✅ 安全加固（HTTPS、CSP、Rate Limiting、Secret Management）
- ✅ 回滚与灾备策略
- ❌ 前端/后端代码（web-frontend / web-backend pack）
- ❌ 测试（web-testing pack）

### 建议的 Capabilities

这个 pack 主要是**代码/工具类**（选型→执行→验证→优化）：

| Capability | 做什么 | 工具 | 产出 |
|-----------|--------|------|------|
| `platform_selection` | 部署平台选型（trade-off 分析） | WebSearch | 选型分析文档 |
| `ci_cd_pipeline` | CI/CD 流水线配置 | GitHub Actions YAML | .github/workflows/*.yml |
| `environment_config` | 环境变量 + 多环境管理 | dotenv, Vercel CLI | .env.example + 环境配置 |
| `domain_dns` | 域名购买 + DNS 配置 + HTTPS | Vercel/Cloudflare CLI | DNS 配置文档 |
| `monitoring` | 错误监控 + 性能监控 + Uptime | Sentry CLI, Lighthouse CI | 监控配置 + 告警规则 |
| `security_hardening` | 安全头 + CSP + Rate Limit + Secrets | Write | 安全配置文档 + 检查清单 |
| `rollback_strategy` | 回滚方案 + 灾备 + 数据备份 | Write + Vercel CLI | 回滚 SOP 文档 |

### 特殊说明：这个 pack 的两种 capability

| 类型 | Capability | Step 模型 |
|------|-----------|----------|
| 文档/研究类 | platform_selection, security_hardening, rollback_strategy | 搜索→分析→推导→生成 |
| 代码/工具类 | ci_cd_pipeline, environment_config, domain_dns, monitoring | 选型→执行→验证→优化 |

**platform_selection 是纯分析**（对比 Vercel/Netlify/Cloudflare/Docker，trade-off 表），用文档类 step 模型。
**ci_cd_pipeline 是纯执行**（写 GitHub Actions YAML），用代码类 step 模型。

### Phase 1 搜索关键词

```
"GitHub claude skills deployment CI CD SKILL.md"
"GitHub claude skills DevOps Vercel Netlify"
"GitHub cursor rules deployment best practices"
"GitHub AI agent CI CD pipeline automation"
"GitHub awesome claude skills deployment infrastructure"
"Vercel deployment best practices 2026"
"GitHub Actions CI CD web app template"
"web application security headers CSP checklist"
"deployment monitoring Sentry setup guide"
"rollback strategy web application best practices"
```
...
| 类别 | 工具 | 说明 | registry 有？ |
|------|------|------|-------------|
| 部署 | Vercel CLI | `npx vercel` — 一键部署 | ❌ 需新增 |
| 部署 | Netlify CLI | `npx netlify deploy` | ❌ 需新增 |
| 部署 | Wrangler (Cloudflare) | `npx wrangler deploy` | ❌ 需新增 |
| CI/CD | GitHub CLI | `gh workflow run` — 触发 Actions | ❌ 需新增 |
| 监控 | Sentry CLI | `npx @sentry/cli` — 错误监控 | ❌ 需新增 |
| 性能 | Lighthouse CI | `npx lhci autorun` — 性能基线 | 需查（testing pack 可能已加） |
| 安全 | Mozilla Observatory | `npx observatory-cli` — 安全头检查 | ❌ 需新增 |
| SSL | certbot | Let's Encrypt 证书 | 需查 |
| 容器 | Docker CLI | `docker build/push` | ❌ 需新增（如果需要） |

**注意**：Vercel CLI 是最核心的工具 — 你的项目大多部署在 Vercel。

### 和其他 Pack 的衔接

```
上游 — Testing 产出:
├── 测试通过报告 → 部署门控（CI 中测试不过不部署）
├── 性能基线 → Lighthouse CI 阈值
└── 覆盖率 → CI 中覆盖率不达标不部署
...
**"把 Todo App 部署到 Vercel"**（延续全链路）：
1. Vercel CLI 部署
2. GitHub Actions CI/CD（push → test → deploy）
3. 环境变量配置（.env.example）
4. Sentry 错误监控配置
5. Lighthouse CI 性能基线

压力测试可以用"多环境 + 自定义域名 + 回滚"场景。

### ⚠️ 这个 pack 的特殊性

和其他 pack 不同，Deployment 的很多操作**需要真实的账号和服务**：
- Vercel 部署需要 Vercel 账号 + token
- Sentry 需要 Sentry 项目
- 域名需要真实域名
- GitHub Actions 需要 repo

**E2E 测试时**：可以只测配置文件生成（.yml, .env.example, vercel.json），不需要真的部署。实际部署在真实项目中验证。
