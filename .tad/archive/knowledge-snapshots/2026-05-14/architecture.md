...
### Capability Pack: 3-Skill Deep Design vs Template Proliferation — session.json Cross-Skill State Flow — 2026-05-07
- **Context**: Building Product Thinking Capability Pack (3 skills: pressure-test, shotgun, define) after web-ui-design. The design question was whether to build 40 thin templates (pm-skills pattern) vs 3 deep interconnected skills.
- **Discovery**: Two capability pack patterns produce qualitatively different outcomes:
  1. **Template proliferation (anti-pattern)**: 40+ SKILL.md files, each 50-100 lines, each covering a distinct workflow step. Users get comprehensive coverage but each skill is shallow — it describes WHAT to do, not HOW to reason. Output is passive (fill-in-the-blank). pm-skills and Product-Manager-Skills both suffer from this pattern.
  2. **Deep skill with interaction contract (preferred)**: 3 SKILL.md files, each 500-800 lines, each with a defined interaction contract (adversarial default tone, mandatory real-data search, explicit pushback patterns). Output is active (decision, not document). The key structural primitives that make deep skills work:
     - **Interaction contract at the top** (Anti-Sycophancy Rules in pressure-test.md): tells the AI HOW to reason, not just what to produce. Without this, LLMs default to sycophantic coaching.
     - **Mandatory search per round**: each forcing round includes explicit search queries the AI must execute. This prevents self-simulation and anchors the AI to current market reality.
     - **session.json for cross-skill state flow**: skills share context through a local JSON file. `/pressure-test` writes; `/shotgun` reads and writes; `/define` reads both and pre-fills 80% of output. This enables the final skill to auto-fill most content from prior steps — "80% auto-fill" is only achievable with persistent intermediate state.
     - **Product type adapter pattern**: separate structure (universal 6 forcing questions) from data layer (type-specific sources, question wording, MVP definition). Same skill file, different behavior per type. Adapters are lightweight (40-120 lines) and keep the main skill focused.
  3. **Session.json schema constraint**: `selected_variant` in the shotgun section is an integer index (not an object). Downstream skills must access `variants[selected_variant-1]` to get the full variant object. This was caught by code-reviewer P0 in Layer 2 — document it explicitly in the schema to prevent repeat mistakes.
- **Action**: When designing a new Capability Pack: (a) start with the interaction contract before writing any workflow steps — decide whether the AI is coach, adversary, or neutral, and encode that in explicit rules at the top of the skill; (b) for multi-skill packs with data flow between skills, design the session.json schema upfront and document all field paths explicitly (not "selected_variant" but "variants[selected_variant-1].reduce"); (c) use the adapter pattern when the same reasoning applies to multiple contexts with different data sources — keep the core skill universal, put the variation in lightweight adapter files; (d) 3 deep skills (500-800 lines each) > 40 thin templates (50-100 lines each) for skills that require sustained reasoning rather than information lookup.
- **Grounded in**: ~/product-thinking/skills/pressure-test.md (Anti-Sycophancy Rules + 6 forcing rounds), ~/product-thinking/skills/define.md (session.json mapping table), ~/product-thinking/adapters/software.md (adapter pattern), .tad/evidence/reviews/blake/capability-pack-product-thinking/code-reviewer.md (P0-2: session.json schema mismatch)
- **Revalidated**: 2026-05-07

### Capability Pack Rule Sourcing: Read the Cited Source, Not Just the Citation — 2026-05-07
- **Context**: Building web-backend capability pack — backend-architect Layer 2 review found 11 P1s, most of which were rules derived from intuition/memory rather than reading the cited source. Examples: timeout formula (p99+50ms → actual SRE Book Ch22 recommendation is p99×2), UUIDv7 timestamp exposure (missed from RFC 9562), SIGTERM-readiness gap (well-known but absent from most graceful-shutdown guides), one-hop rule arithmetic wording error.
- **Discovery**: The pattern: Blake wrote rule text with a `[Source: repo-name]` attribution tag but never fetched or read the actual source before writing the rule. The attribution was correct (the source does discuss the topic) but the rule text was generated from Blake's training-data intuition, not from reading the source. This produced plausible-sounding rules with subtle errors — cursor pagination missing compound-cursor caveat, HSTS missing first-deployment ramp-up guidance, etc.
  - **Root cause differs from invention**: The original "Domain Pack Research" lesson (2026-04-03) was about packs listing tools without usage commands — outright invention. This new failure mode is subtler: the source is real and relevant, but the rule text is synthesized from memory rather than derived from the source. A rule can be 80% correct and still ship with a production-critical gap.
  - **Verification asymmetry**: These errors are invisible to pre-handoff Alex Gate 2 review (reviewer reads the handoff spec, not the cited source) and to code-reviewer Layer 2 (checks code correctness, not domain accuracy). Only backend-architect with domain expertise caught them.
- **Action**: For capability pack rule authoring, Blake must: (a) for each rule with a `[Source: X]` tag, WebFetch or `gh api` the actual source file and read the relevant section before writing the rule text; (b) if the source cannot be read (private repo, broken URL), mark as `[Source: X — unverified, derived from training data]`; (c) for rules involving specific numeric thresholds (timeouts, page sizes, retry counts), always trace the number to a primary source document.
- **Grounded in**: ~/web-backend/references/database.md (p99×2 timeout fix), ~/web-backend/references/infrastructure.md (SIGTERM-readiness gap), ~/web-backend/references/api-design.md (compound cursor caveat, HSTS ramp), .tad/evidence/reviews/blake/capability-pack-web-backend/backend-architect.md (P1-1 through P1-8)
- **Revalidated**: 2026-05-07

### Kubernetes SIGTERM-Readiness Race: preStop Sleep is Mandatory for Zero-Downtime Deploys — 2026-05-07
- **Context**: web-backend capability pack infrastructure.md Rule 7 (graceful shutdown). Backend-architect P1-8 catch: all graceful shutdown guides show "handle SIGTERM → close server" but miss the Kubernetes-specific timing gap.
- **Discovery**: In Kubernetes, the sequence is: (1) kubelet sends SIGTERM to pod, (2) kubelet removes pod from Service endpoints, (3) kube-proxy propagates endpoint removal to all nodes. Steps (1) and (2) happen simultaneously, but step (3) takes 1-3s to propagate across all nodes. During the propagation window, kube-proxy on other nodes still routes new connections to the pod — which may have already called `server.close()` and is no longer listening → `ECONNREFUSED` to end users during a rolling deployment.
  - **Fix**: Add a `preStop` lifecycle hook that sleeps before SIGTERM is sent, giving endpoint removal time to propagate: `lifecycle: { preStop: { exec: { command: ["sh", "-c", "sleep 10"] } } }`. The `terminationGracePeriodSeconds` must be > the preStop sleep + shutdown time.
  - **Why it's widely missed**: The Kubernetes docs and most graceful shutdown guides focus on in-process connection draining, not on the inter-node propagation delay. The `preStop` hook is documented but its necessity for zero-downtime deployments is not prominently explained.
...
### Shell Dispatcher: `set -e` + Exit-10 Propagation, Portable Timeout, and Set-Diff Source ID — 2026-05-09
- **Context**: NotebookLM Source Preprocessor Pipeline (TASK-20260509-001) — 5-file shell dispatcher with 4 handler types and non-zero exit codes for two success paths (0=.md, 10=URL).
- **Discovery**: Four load-bearing shell patterns emerged:
  1. **`set -e` propagates non-zero handler exits through `case` arms**: A handler inside a `case` statement that exits 10 causes the parent dispatcher to exit 10 immediately (no additional `exit` call needed). The child's stdout flows through automatically. This makes exit-code-based dispatch clean without explicit plumbing.
  2. **Portable timeout wrapper on macOS**: macOS ships without GNU `timeout`. The portable pattern is `run_with_timeout() { if gtimeout available → use gtimeout; elif timeout available → use timeout; else → run without cap (no-op fallback) }`. Never hardcode `timeout cmd` — it exits 127 on stock macOS and `set -e` causes silent total failure.
  3. **Set-difference for newly-added item identification**: When an append-only list (NotebookLM `source list --json`) doesn't guarantee insertion order, identify a newly-added item via: snapshot IDs before add → add → snapshot IDs after → `comm -13 <(sorted_before) <(sorted_after) | head -1`. Relying on `.[-1]` (last element) is fragile and can cause silent wrong-source operations (data deletion risk).
  4. **UTM param normalization — per-param split, not bulk regex**: `sed -E 's/[?&]utm_*=...//g'` silently corrupts URLs where utm_* is the FIRST query param (e.g., `?utm_source=x&id=42` → `&id=42`, losing the `?`). The reliable pattern: `printf '%s' "$query" | tr '&' '\n' | grep -v '^utm_' | tr '\n' '&' | sed 's/&$//'` — split into per-param lines, filter, rejoin. Same class of "clever regex hides a data corruption bug" as the stripping-too-much anti-pattern.
...
- **Context**: LSP Code Understanding Integration inserted `1_5d_lsp_blast_radius` between `1_5c_research_task_detection` and `1_6_tdd_check`. Backend-architect P0-1 caught that 1_5c's explicit exit path (line 575) said `proceed to 1_6_tdd_check` — written before 1_5d existed. This stale arrow silently bypassed 1_5d for ALL non-research tasks (the common case where blast radius analysis is most valuable).
- **Discovery**: When inserting a new step between existing steps in a protocol state machine, updating the new step's `trigger` field ("After X, before Y") is necessary but NOT sufficient. ALL explicit transition arrows in predecessor steps must also be audited and updated. The failure mode: step N has a `trigger: "After N-1, before N+1"` which is correct, but step N-1's action text still says `proceed to N+1`, causing the new step N to be skipped entirely. This is the same class as "Protocol State-Machine Design" (2026-05-02 entry) applied specifically to step INSERTION rather than initial design.
  - **Audit pattern for step insertions**: `grep -n 'proceed to\|skip.*proceed\|goto\|continue to' SKILL.md | grep -i '<successor_step_name>'` — find all explicit references to the step that USED to be the successor. Each hit is a candidate for update.
  - **Why expert review catches it but self-check misses it**: Blake's Layer 1 AC verification checks that the new step EXISTS (grep -c), not that it's REACHABLE from all predecessor paths. The transition graph requires reading predecessor step action text, which is outside AC verification scope.
- **Action**: When inserting step N between N-1 and N+1: (1) update N's trigger field (already standard); (2) grep for ALL explicit references to N+1 in N-1's action text and update them to N; (3) grep for references to N+1 in ANY step that might have a conditional skip path past N-1 (e.g., 1_5b might also have a "proceed to 1_6" arrow). The grep audit is cheap (~2 minutes) and prevents silent step-bypass bugs that are invisible to AC-based verification.
- **Grounded in**: .claude/skills/blake/SKILL.md line 575 (fixed from `1_6_tdd_check` to `1_5d_lsp_blast_radius`), .tad/evidence/reviews/blake/lsp-code-understanding/backend-architect.md (P0-1 finding)
- **Revalidated**: 2026-05-14

### Epic Auto-Conductor: Sub-Agent Nesting Limit and Audit Chain Discipline — 2026-05-14
- **Context**: Spike testing a "Conductor" pattern where one main session orchestrates Alex and Blake as sub-agents, eliminating human copy-paste between terminals. Two spikes on menu-snap (multilingual expansion, 2 Phases, 52 files changed).
- **Discovery**: Five load-bearing findings from two spikes:
  1. **Sub-agents have NO Agent tool**: Claude Code sub-agents spawned via Agent tool cannot spawn their own sub-agents. This means Alex sub-agent cannot call code-reviewer, Blake sub-agent cannot run Layer 2 expert review. All independent review must happen at the Conductor (main session) level — Conductor is the ONLY entity that can spawn reviewer sub-agents.
  2. **Sub-agents fabricate reviewer labels**: When Alex sub-agent is asked to do "expert review," it generates findings from its own analysis and labels them as "CR-P0-1 (code-reviewer)" and "FA-P0-1 (frontend-architect)." These labels look like independent review but are self-assessment. Conductor must NOT trust these labels — it must run its own independent review.
  3. **File is source of truth, prompt is not**: When Conductor's reviewer found P0s (4 missing files in handoff), the fix was passed to Blake via prompt text instead of writing it back to the HANDOFF.md file on disk. Result: HANDOFF.md stayed wrong, review findings were ephemeral, audit trail was broken. Correct pattern: Conductor writes REVIEW.md → updates HANDOFF.md → Blake reads HANDOFF.md from disk. Prompt only carries file path, never business content.
  4. **worktree isolation doesn't protect downstream projects**: Agent tool's `isolation: "worktree"` creates a worktree in the TAD repo (current working directory), but if Blake modifies files in a downstream project (e.g., menu-snap), those changes go directly to the downstream project's main branch. True isolation requires Conductor to create a feature branch in the target project before dispatching Blake.
  5. **Conductor must ground before dispatching Alex**: The main session has context about the target project's codebase (from previous reads in the conversation). Alex sub-agent starts fresh with no project knowledge. Conductor must read relevant code BEFORE spawning Alex and pass grounded context (actual code patterns, line numbers, import conventions) in the prompt. Without grounding, Alex designs against hallucinated code structure.
- **Architecture principle**: Conductor = PM (Socratic with human) + Quality auditor (only real reviewer layer) + Dispatcher (manages Alex/Blake). Alex = pure architect (no review capability). Blake = pure engineer (no review capability). Human = goal-setter + final accepter.
- **Action**: Any future Conductor implementation must enforce: (a) every Conductor step produces a persistent file (REVIEW.md, updated HANDOFF.md, GATE-REPORT.md); (b) sub-agent instructions reference file paths, not inline content; (c) Conductor runs its own independent review after each sub-agent completes; (d) Alex/Blake "self-review" findings are advisory, not counted as formal review.
- **Grounded in**: .tad/active/ideas/IDEA-20260514-epic-auto-conductor.md (full spike report), menu-snap commits 332a948 + 0cec51c (real implementation), HANDOFF-20260514-i18n-phase1.md + phase2.md (persistent artifacts)
- **Revalidated**: 2026-05-14

### Sufficiency Check Must Precede the Step It Influences — 2026-05-14
- **Context**: Epic Template Enhancement added a "sufficiency check" in `epic_linkage` (handoff_creation_protocol.step1) that was supposed to reduce Socratic inquiry. Both code-reviewer and backend-architect independently caught that epic_linkage runs AFTER Socratic has completed — the check cannot retroactively reduce questions already asked.
- **Discovery**: When a protocol introduces a conditional that modifies the behavior of an earlier step (e.g., "reduce Socratic to light tier"), the conditional must be placed BEFORE that earlier step, not co-located with the logic that uses the result. The fix was adding `step2b_phase_detail_check` between Epic Assessment and Socratic. Two related sub-findings:
  1. **Handoff design can have ordering bugs**: Alex's handoff explicitly specified placing the check in epic_linkage. Expert review at Gate 2 did not catch the ordering issue (same reviewers who found it post-impl). Pre-handoff review evaluates spec correctness in isolation; post-impl review evaluates against the actual protocol execution flow. This is the same asymmetry as "Pre-Handoff vs Post-Implementation Reviewer" (2026-04-27).
  2. **Status state machines across parallel data structures must be synchronized**: Phase Map table and Phase Detail Block both track status (Planned/Active/Done) but are updated by different protocol steps. Without explicit synchronization (step 3b adds Active to Detail Block when step 3 adds Active to Phase Map), the two data structures diverge during the entire Phase execution window.
- **Action**: When adding a conditional that says "if X, then reduce/skip step Y", verify that the conditional check runs BEFORE step Y in the protocol execution order. When two data structures track the same lifecycle state, add synchronization at every state transition (not just at creation and completion).
- **Grounded in**: .claude/skills/alex/SKILL.md (step2b_phase_detail_check, epic_linkage step 3b), .tad/evidence/reviews/blake/epic-template-enhancement/backend-architect.md (P0-1, P0-3), .tad/evidence/reviews/blake/epic-template-enhancement/code-reviewer.md (P0-1, P0-2)
- **Revalidated**: 2026-05-14

### Autonomous Protocol Design Requires Three Mandatory Patterns — 2026-05-14
- **Context**: Building YOLO execution protocol — autonomous multi-phase Epic execution without human in the loop. Both code-reviewer and backend-architect independently found critical gaps in the initial implementation.
- **Discovery**: Manual-mode protocols can rely on humans to course-correct at dead-ends and handle unexpected failures. Autonomous protocols cannot. Three patterns are mandatory for any protocol designed to run unattended:
  1. **Explicit transition arrows at every step**: Every step's action block must end with `→ Proceed to step_YN` or a branching decision (`If X → step_A, Else → step_B`). Without this, different model versions/context states make different navigation decisions. BA P0-1 found 8 steps with no exit arrows. Architecture.md "Protocol State-Machine Design" (2026-05-02) already documented this for initial design, but it's doubly critical for autonomous execution where no human resolves dead-ends.
  2. **Verify + on_verify_fail for every sub-agent output**: Sub-agents can fail silently (produce incomplete output, crash, hit token limits). Every sub-agent spawn needs: (a) a `verify` check on the output (file exists, minimum content), (b) an `on_verify_fail` path (retry with context, circuit breaker, honest_partial). BA P0-3 caught that Y3 had verify but no failure path.
  3. **Re-review after every P0 fix**: When a reviewer finds P0 and the Conductor fixes it, the fix must be re-reviewed. Otherwise the Conductor declares Gate PASS based on reviews of v1, not v2. BA P0-4 caught this in both Y4 (design review) and Y6 (impl review). Production TAD's manual mode handles this naturally (expert review Round 2), but autonomous mode must make it explicit.
  - **Sub-finding**: KEEP steps from the manual-mode mapping table that require tool access (LSP, Bash, domain pack YAML reading) cannot be delegated to sub-agents — sub-agents may lack the tools. These must be Conductor-side post-validation steps (CR P0-1 led to step_Y3b).
  - **Sub-finding**: `subagent_type` parameter determines the sub-agent's persona. Using `backend-architect` for both design (Y3) and review (Y4) undermines independence. Design sub-agent should be `general-purpose`; reviewer sub-agents should be specialized (BA P0-2).
- **Action**: When designing any autonomous protocol: (a) add transition arrows as the LAST line of each step (not a separate section — inline so they can't be separated from the step during editing); (b) every `Agent` spawn needs `verify` + `on_verify_fail` (retry→circuit-breaker→honest_partial); (c) every P0-fix loop needs a re-review step before proceeding; (d) classify manual-mode KEEP steps by tool requirements — Conductor-only steps go to a post-validation step, not into the sub-agent prompt.
- **Grounded in**: .claude/skills/alex/SKILL.md (yolo_execution_protocol Y1-Y8 + Y3b), .tad/evidence/reviews/blake/yolo-execution-mechanism/backend-architect.md (P0-1 through P0-4), .tad/evidence/reviews/blake/yolo-execution-mechanism/code-reviewer.md (P0-1, P0-2)
- **Revalidated**: 2026-05-14

### Cross-Model CHALLENGE_INSTRUCTION Must Be a Defined Constant — 2026-05-14
- **Context**: Research Adversarial Challenge layer (Phase 0c/4c/5b) invokes both Codex and Gemini with the same challenge prompt. Initial implementation had subtly different wording per model.
- **Discovery**: Prompt symmetry between cross-model invocations must be enforced mechanically — define the instruction string ONCE as `CHALLENGE_INSTRUCTION` and pass via `$CHALLENGE_INSTRUCTION` to both models. Without this, experiment comparisons are invalidated because inputs differ. This extends the 2026-05-03 "Prompt Symmetry is Load-Bearing" entry from a design principle to an implementation pattern.
- **Action**: For any TAD protocol that sends the same task to multiple external models: define the prompt as a single constant at protocol entry, reference by variable in all invocation sites.
- **Grounded in**: .claude/skills/alex/SKILL.md (CHALLENGE_INSTRUCTION in research_plan_protocol Phase 0c), .tad/evidence/completions/research-adversarial-challenge/COMPLETION-20260514-research-adversarial-challenge.md
- **Revalidated**: 2026-05-14
