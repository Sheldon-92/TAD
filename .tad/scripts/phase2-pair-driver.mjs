...
const PAIRS_DIR = path.join(ROOT, '.tad/evidence/yolo/yolo2-verified-orchestration/phase2/pairs');
...
function setupRepo(task, arm, pairDir) {
  ...
  const goal = {
    ...
    quality_policy: { phase_candidate_requires_hidden_acceptance: false, phase_candidate_requires_alignment: true, wrong_or_unauthorized_next_action_max: 0, repeated_verified_action_max: 0 },
  };
...
function contractFile(dir, sl, task) {
  ...
  const targets = Object.keys(task.seed);
  fs.writeFileSync(path.join(dir, rel), JSON.stringify({
    format: 'yolo-slice-contract-v1', slice_id: sl.id, outcome: sl.outcome,
    maps_to_success: ['SC-1'], necessary_evidence: [], allowed_paths: targets,
    forbidden_scope_sha256: shas(JSON.stringify(task.hidden)), tool_allowlist: ['Read', 'Edit', 'Write'],
...
function executionTurn(repo, hostEv, roundId, session, nonce, sl, task) {
  ...
  fs.writeFileSync(pfile, prompt);
  const args = ['turn', '--host-evidence', hostEv, '--packet', `.tad/evidence/yolo/run/rounds/${roundId}/execution.md`, '--prompt', pfile, '--role', 'executor', '--turn-kind', 'execution', '--sandbox', 'workspace-write'];
  if (session) args.push('--session', session);
...
function runArm(task, arm, pairDir) {
  ...
  if (fs.existsSync(marker)) { console.log(`  ${arm}: already done`); return JSON.parse(fs.readFileSync(marker, 'utf8')); }
  ...
  for (let i = 0; i < sls.length; i += 1) {
    ...
    fs.writeFileSync(aJson, JSON.stringify({ format: 'yolo-recovery-assertion-v1', verdict: score.verdict, author_id: `codex:${sessionId}`, hard_correct: score.hard, hard_total: 8, soft_score: score.soft, packet_sha256: aRec.packet_sha256 }));
    const rJson = path.join(hostEv, `review-${rid}.json`);
    fs.writeFileSync(rJson, JSON.stringify({ format: 'yolo-recovery-review-v1', verdict: score.verdict, reviewer_id: 'deterministic-rubric-v1' }));
    ...
    const targetFile = Object.keys(task.seed)[0];
    const targetAbs = path.join(dir, targetFile);
    const preSha = shaF(targetAbs);
    const argsJsonP = path.join(hostEv, `args-${rid}.json`);
    fs.writeFileSync(argsJsonP, JSON.stringify({ op: 'governed-slice-edit', target: targetFile }));
    const effP = path.join(hostEv, `effects-${rid}.json`);
    fs.writeFileSync(effP, JSON.stringify({ affected_real_paths: [targetFile] }));
    const asRes2 = cli(dir, ['action-start', '--run', '.tad/evidence/yolo/run', '--action', `A-${rid}`, '--description', `governed edit of ${targetFile} for ${sl.id}`, '--target', targetFile, '--pre-sha256', preSha, '--intended-post-sha256', shas(String(sl.outcome)), '--round', rid, '--outcome-id', `OID-${rid}`, '--tool', 'Edit', '--args-json', argsJsonP, '--effect-manifest', effP]);
...
${asRes2.out.slice(-400)}`);
    ...
    if (outcome === 'reconciled') {
      const evP = path.join(hostEv, `reconcile-evidence-${rid}.json`);
      fs.writeFileSync(evP, JSON.stringify({ note: 'post-state diverged from frozen prediction; explicit evidence attached', observed_sha256: postSha }));
      recArgs.push('--evidence', evP);
    }
    rec(dir, recArgs);
    // Close candidate (reconcile actions if any pending).
    const reportP = path.join(hostEv, `report-${rid}.json`);
    fs.writeFileSync(reportP, JSON.stringify({ format: 'yolo-round-report-v1', round_id: rid, changed_paths: eRec.tool_calls.flatMap(c => c.observed_changed) }));
  ...
  const result = { arm, task: task.id, final_hidden_acceptance: finalHidden[finalHidden.length - 1], rounds: evidence.rounds, tokens_total: evidence.rounds.reduce((s, r) => s + r.tokens, 0) };
...
// ── main ──
const index = JSON.parse(fs.readFileSync(path.join(PAIRS_DIR, 'dataset-index.json'), 'utf8'));
const results = [];
for (const p of index.pairs) {
  console.log(`PAIR ${p.pair_id} (${p.task_id})`);
  const pairDir = path.join(PAIRS_DIR, p.pair_id);
  fs.mkdirSync(pairDir, { recursive: true });
  const task = JSON.parse(fs.readFileSync(path.join(PAIRS_DIR, p.task_id, 'task.json'), 'utf8'));
  const ctrl = runArm(task, 'control', pairDir);
  const trt = runArm(task, 'treatment', pairDir);
  const pairResult = {
    pair_id: p.pair_id, task_id: p.task_id,
...
fs.writeFileSync(path.join(PAIRS_DIR, '..', 'pair-results.json'), JSON.stringify(out, null, 2));
console.log('ALL PAIRS COMPLETE ->', path.join(PAIRS_DIR, '..', 'pair-results.json'));
