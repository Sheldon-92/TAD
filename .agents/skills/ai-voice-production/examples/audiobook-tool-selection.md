...
```bash
grep -oE 'reference duration|minimum reference|3s|5s|6s|10s|15s reference|ACX|44\.1 ?kHz|192 ?kbps|RMS -2[0-9]|-23 to -18|\.pt|voice identity|speaker embedding|seed reset|16GB|MPS|PYTORCH_ENABLE_MPS_FALLBACK|float32' audiobook-tool-selection-output.md | sort -u | wc -l | tr -d ' '
# Expected: ≥ 3
```
...
- ✅ "ACX 44.1kHz / 192kbps / RMS -23 to -18 dB" (the pack's exact Audible mastering spec)
