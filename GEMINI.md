...
- **Context syntax**: Use `@{file}` for file references, `!{command}` for shell output

---

## Command Reference

| TAD Command | Gemini Equivalent |
|-------------|-------------------|
| /alex | /tad-alex |
| /blake | /tad-blake |
| /gate | /tad-gate |
...
## Gemini Context Syntax

Use these special syntaxes for context injection:
...
检测到 handoff 文件读取
     ↓
必须立即调用 /tad-blake 进入执行模式
     ↓
执行实现
     ↓
必须执行 /tad-gate 3 (Implementation Quality)
     ↓
必须执行 /tad-gate 4 (Integration Verification)
     ↓
完成交付
```
...
**Gemini CLI 使用 self-check 模式执行技能审查，而非调用 subagents。**
...
规则: Alex 写完 handoff 后，必须停止并等待人类传递信息给 Blake
      不能在同一个 terminal 调用 /tad-blake
      人类是 Alex 和 Blake 之间唯一的信息桥梁

流程:
  Terminal 1 (Alex):
    1. 需求分析 → 苏格拉底式提问 → 设计 → 写 handoff
    2. 输出: "Handoff 已创建，请在 Terminal 2 执行 /tad-blake"
    3. 停止，等待人类反馈
...
## Memory Commands (Gemini-specific)

Gemini CLI supports memory commands for context management:

- `/memory show` - Display current memory/context
- `/memory refresh` - Refresh context from files
