# Handoff Template: Create New Domain Pack — Web Testing
...
为 **Web Testing** 创建一个 Domain Pack — 一个 YAML 配置文件，让 TAD 能在这个领域"做事"。
...
执行这 5 条 WebSearch（把 web-testing 替换为具体领域）：

```
1. "GitHub claude skills web-testing SKILL.md"
2. "GitHub AI agent web-testing skill prompt framework"
3. "GitHub awesome claude skills web-testing"
4. "web-testing best practices checklist framework"
5. "site:github.com web-testing SKILL.md OR CLAUDE.md"
```
...
```markdown
# web-testing Skills Best Practices
...
**写入**：`.tad/spike-v3/domain-pack-tools/web-testing-skills-best-practices.md`
...
Agent({
  ...
  读取 .tad/domains/web-testing.yaml 的 {capability} 部分，按 steps 逐步执行。
...
| # | 文件 | 检查 |
|---|------|------|
| 1 | `.tad/spike-v3/domain-pack-tools/web-testing-skills-best-practices.md` | ≥3 仓库 × 5 维度 |
| 2 | `.tad/spike-v3/domain-pack-tools/web-testing-tool-research.md` | ≥2 工具实测 |
| 3 | `.tad/domains/tools-registry.yaml` 更新 | 新工具有完整条目 |
| 4 | `.tad/domains/web-testing.yaml` | 四层 steps + 质量标准 |
...
## Alex 的领域补充：Web Testing 特定指引

### 范围定义

**Web Testing Pack 覆盖所有测试层级，包括人机协作测试。**
- ✅ E2E 测试（Playwright 自动化、用户流程验证）
- ✅ 单元/集成测试（Vitest/Jest、组件测试）
- ✅ 性能测试（Lighthouse、Core Web Vitals）
- ✅ 可访问性测试（pa11y、axe）
- ✅ API 测试（端点验证、契约测试）
- ✅ **人机协作测试（Pair Testing）** — TAD 已有 4D Protocol 经验
- ✅ 视觉回归测试
- ❌ 代码编写（web-frontend/backend pack）
- ❌ 部署（web-deployment pack）

### 建议的 Capabilities

这个 pack 有两类能力 — **自动化测试**和**人机协作测试**：

#### 自动化测试（代码类 — 选型→执行→验证→优化）

| Capability | 做什么 | 工具 | 产出 |
|-----------|--------|------|------|
| `e2e_testing` | 用户流程自动化测试 | Playwright | 测试脚本 + 执行报告 |
| `unit_testing` | 组件/函数单元测试 | Vitest | 测试文件 + 覆盖率报告 |
| `api_testing` | API 端点验证 + 契约测试 | curl/httpie + OpenAPI diff | 测试脚本 + 结果 |
| `performance_testing` | 性能评分 + Core Web Vitals | Lighthouse CLI, unlighthouse | 性能报告 + 优化建议 |
| `accessibility_testing` | WCAG 合规检查 | pa11y, axe | 违规报告 + 修复建议 |

#### 人机协作测试（混合类 — 这是 TAD 的特色）

| Capability | 做什么 | 方式 | 产出 |
|-----------|--------|------|------|
| `pair_testing` | 人类 + AI 协作 E2E 测试 | 4D Protocol (Discover→Discuss→Decide→Deliver) | 测试报告 + 修复决策 |
| `test_strategy` | 测试策略设计（选什么测、怎么测、测多少） | 文档类：搜索→分析→推导→生成 | 测试计划文档 |

### ⚠️ 关键经验：4D Pair Testing Protocol

TAD 已有配对测试经验（4D Protocol）。这是我们验证过的人机协作测试方法：

```
4D Protocol:
  Discover — AI 发现问题（截图 + 分析）
  Discuss  — 人和 AI 讨论问题的严重性和原因
  Decide   — 当场决定修还是不修、怎么修
  Deliver  — 记录决策，生成修复 handoff
```

**为什么 Pair Testing 比纯自动化好**（用户的实际经验）：
- 人直觉知道哪些功能 work 哪些不 work — 这是自动化测不出来的
- AI 能同时看代码和截图，发现人不容易注意到的问题
- 当场决策比"列 bug 清单后再 triage"更高效（1M context 保证不丢信息）

**这个 capability 应该是 web-testing pack 的核心差异化** — 市面上所有测试 skills 都是纯自动化的，没有人机协作。

### Phase 1 搜索关键词

```
"GitHub claude skills testing Playwright E2E SKILL.md"
"GitHub claude skills test strategy QA automation"
"GitHub cursor rules testing best practices"
"GitHub AI agent testing code generation Playwright"
"GitHub awesome claude skills testing QA"
"Playwright best practices test structure 2026"
"testing strategy checklist web application"
"pair testing AI human collaboration"
"Lighthouse CI performance budget CLI"
"visual regression testing AI tools"
```
...
| 类别 | 工具 | 说明 | registry 有？ |
|------|------|------|-------------|
| E2E | Playwright | `npx playwright test` — 82k 搜索/月，最热 MCP | 需查 |
| 单元 | Vitest | `npx vitest run` — 速度快，兼容 Jest | 需查 |
| 性能 | Lighthouse CI | `npx lhci autorun` — 性能分自动化 | 需查 |
| 性能全站 | unlighthouse | `npx unlighthouse --site url` — 全站性能扫描 | ✅ 已有 |
| 可访问性 | pa11y | `npx pa11y url` — WCAG 检查 | ✅ 已有 |
| 可访问性 | axe | `npx @axe-core/cli url` — 更全面 | 需查 |
| API 测试 | curl/httpie | 内置 | ✅ 已有 |
| 覆盖率 | c8/istanbul | `npx vitest --coverage` | 需查 |
| 视觉回归 | Playwright screenshot | Playwright 内置截图对比 | 需查 |
| Mock | MSW | `npx msw init` — Mock Service Worker | 需查 |

**注意**：Playwright 是这个 pack 的核心工具（82k 搜索/月，整个 MCP 生态最热）。

### 和其他 Pack 的衔接

```
上游 — Frontend + Backend 产出:
├── 可运行的应用 → E2E 测试对象
├── API 端点 → API 测试对象
├── 组件代码 → 单元测试对象
└── 性能基线（build size）→ 性能测试基准
...
**"测试 Todo App 的全部功能"**（延续全链路）：
1. E2E：Playwright 跑 CRUD 用户流程
2. 单元：Vitest 测核心组件
3. 性能：Lighthouse 评分
4. 可访问性：pa11y 扫描
5. API：curl 验证端点

压力测试可以用 SaaS PM Tool。

### ⚠️ 这个 pack 的独特价值

市面上测试 skills 全是纯自动化（"写 Playwright 脚本"）。Web Testing pack 的差异化：
1. **test_strategy** — 不只是执行测试，先设计测试策略（测什么、不测什么、为什么）
2. **pair_testing** — 人机协作，4D Protocol，当场发现+决策（TAD 独有）
3. **工具链完整** — 从 E2E 到单元到性能到可访问性，不是只有 Playwright
