# Handoff Template: Create New Domain Pack — Web UI Design
...
为 **Web UI Design** 创建一个 Domain Pack — 一个 YAML 配置文件，让 TAD 能在这个领域"做事"。
...
执行这 5 条 WebSearch（把 web-ui-design 替换为具体领域）：

```
1. "GitHub claude skills web-ui-design SKILL.md"
2. "GitHub AI agent web-ui-design skill prompt framework"
3. "GitHub awesome claude skills web-ui-design"
4. "web-ui-design best practices checklist framework"
5. "site:github.com web-ui-design SKILL.md OR CLAUDE.md"
```
...
```markdown
# web-ui-design Skills Best Practices
...
**写入**：`.tad/spike-v3/domain-pack-tools/web-ui-design-skills-best-practices.md`
...
Agent({
  ...
  读取 .tad/domains/web-ui-design.yaml 的 {capability} 部分，按 steps 逐步执行。
...
| # | 文件 | 检查 |
|---|------|------|
| 1 | `.tad/spike-v3/domain-pack-tools/web-ui-design-skills-best-practices.md` | ≥3 仓库 × 5 维度 |
| 2 | `.tad/spike-v3/domain-pack-tools/web-ui-design-tool-research.md` | ≥2 工具实测 |
| 3 | `.tad/domains/tools-registry.yaml` 更新 | 新工具有完整条目 |
| 4 | `.tad/domains/web-ui-design.yaml` | 四层 steps + 质量标准 |
...
## Alex 的领域补充：Web UI Design 特定指引

### 范围定义

**Web UI Design Pack 只做设计，不做开发。** 范围：
- ✅ 信息架构（页面结构、导航逻辑）
- ✅ 线框图（布局、组件位置、交互流程）— 含 HTML 原型（视为设计产物，非交付代码）
- ✅ 视觉设计（色彩、字体、组件样式）
- ✅ 交互设计（状态变化、动效、反馈）
- ✅ 设计系统（Design Tokens、组件规范）
- ✅ 可用性审查
- ❌ 前端代码实现（那是 web-frontend pack）
- ❌ 后端 API（那是 web-backend pack）
- ❌ 部署（那是 web-deployment pack）

### 建议的 Capabilities

| Capability | 做什么 | 关键产出 |
|-----------|--------|---------|
| `information_architecture` | 定义页面结构、导航、用户流程 | 站点地图 + 用户流程图（D2/Mermaid） |
| `wireframing` | 低保真线框图（布局、组件、交互） | HTML 原型或 Figma 线框图 |
| `visual_design` | 色彩、字体、组件视觉规范 | Design Tokens 文档 + 样式指南 PDF |
| `interaction_design` | 状态定义、转场、微交互 | 交互规范文档 + 状态图 |
| `design_system` | 可复用的组件库定义 | 组件清单 + 使用规范 |
| `responsive_design` | 响应式/自适应设计（断点、移动端优先） | 断点规范 + 各尺寸线框图 |
| `usability_review` | 设计审查（可用性、可访问性） | 审查报告 + 改进建议 |

### Phase 1 搜索关键词

```
"GitHub claude skills UI design UX wireframe SKILL.md"
"GitHub AI agent UI design system figma workflow"
"GitHub cursor rules UI UX design frontend"
"GitHub claude skills design tokens component library"
"UI design checklist best practices accessibility WCAG"
"information architecture sitemap user flow AI agent"
"design system documentation template 2026"
"GitHub awesome claude skills figma design"
```
...
| 类别 | 搜索什么 | 可能的工具 |
|------|---------|-----------|
| 线框图 | Figma MCP, Frame0 MCP, MockFlow MCP | 直接生成可编辑线框图 |
| HTML 原型 | Write (内置) | HTML+CSS 原型，浏览器直接预览 |
| 流程图/站点地图 | D2 (已有), Mermaid | 用户流程、页面结构 |
| Design Tokens | Style Dictionary CLI | 生成跨平台 design tokens |
| 色彩工具 | 无需外部工具 | Claude 可以直接推荐色彩方案 + 生成 CSS 变量 |
| 可访问性检查 | pa11y CLI, axe CLI | 自动化 WCAG 检查 |
| 可访问性 | axe-core CLI | WCAG 自动检查（需要 HTML 原型作为输入） |

**注意**：D2、Typst、Pandoc 已在 registry 中。重点搜 **UI 设计特有**的工具（Figma MCP、Frame0、pa11y、Style Dictionary）。

### 和其他 Pack 的关系

```
Product Definition  →  Web UI Design  →  Web Frontend  →  Web Testing  →  Web Deployment
     (定义产品)        (设计界面)         (实现代码)       (测试质量)       (部署上线)
```

Web UI Design 的产出（线框图、设计规范、Design Tokens）是 Web Frontend 的输入。

### 测试议题建议

**"一个待办事项 App 的 UI 设计"** — 够小（单页）、够具体（人人知道 Todo App 长什么样）、够有变化空间（可以探索不同设计方向）。

测试时验证：
1. 能生成站点地图/用户流程图（D2）？
2. 能生成线框图（HTML 原型或 Figma MCP）？
3. 能输出 Design Tokens（CSS 变量或 JSON）？
4. 能做可用性审查（有 checklist 不是空话）？
