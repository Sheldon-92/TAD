...
tests_rules:
  ...
  - "SUP3: Swarm failure surface O(n²) = n(n-1)/2 (10 agents = 45 pathways)"
...
# that a no-pack agent would NOT (the cliff formula, the n(n-1)/2 surface, the
# 63.4% figure, event-sourcing replay, the 20-40% supervisor tax).
discriminative_pattern: "1 ?- ?\\(1 ?- ?p\\)\\^s|63\\.4%|99\\.3%|n\\(n ?- ?1\\) ?/ ?2|O\\(n.?2\\)|event[ -]sourc|complexity cliff|20.?40% token|8.?12 (round|turn)"
...
2. **O(n²) swarm failure surface** [structural]: the agent applies n(n-1)/2 and names the pathway count for the stated agent count
   grep pattern: `n\(n ?- ?1\) ?/ ?2|O\(n.?2\)|45 (pathways|failure)|quadratic`
...
```bash
grep -oE '1 ?- ?\(1 ?- ?p\)\^s|63\.4%|99\.3%|complexity cliff|n\(n ?- ?1\) ?/ ?2|O\(n.?2\)|quadratic|8.?10 (sequential )?(agent )?turn|semantic drift|event[ -]sourc|Temporal|20.?40% token|8.?12 (round|turn)' agent-orchestration-fixture-output.md | sort -u | wc -l | tr -d ' '
# Expected: ≥ 4
```
...
- ✅ "n(n-1)/2 → 45 failure pathways for 10 agents" (the pack's quadratic swarm failure-surface law [23])
...
- ❌ "10 agents may be too many" (restates the input without the n(n-1)/2 law)
