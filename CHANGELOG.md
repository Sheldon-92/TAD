...
- **默认通道改回 full（`/alex` `/blake` `/gate`）**。lite（`/alex-lite` `/blake-lite`）
  自 2026-08-13 起标记为**已冻结的实验**：不接新工作，显式调用仍完全可用。
  此前 11 处文件把 lite 写成默认，新项目会被自动导进冻结通道。
- **Alex 激活即付从 107.7K 降到 62K tokens（−42%），纪律一条未减。** 四刀：
  路由脱钩 → 纪律清单补全（15 → 30 条）→ 地板表（每条纪律标必须常驻还是可按需）
  → 启动扫描改为只跑命令读输出（−42.9K）→ 按地板表懒加载（模式专属块外置 + 存根压缩）。
- **29 条义务型约束的祈使句原文进入常驻层**（`alex/SKILL.md` 激活协议之前）。
  判据是「被禁的动作是不是 agent 的默认行为」：忘了就等于跳过的（专家审查、研究先行、
  致命操作人审）必须留一行原文，不能只留 `load_when` 指针。
- **致命操作识别表移到 `config-quality.yaml`**（原在 `config-cognitive.yaml`，
  而 Alex 的 STEP 3 从不加载该模块）。此前 agent 答得出「必须先经人审」，
  **列不出识别特征**——知道要停，认不出什么该停。

### 修复

- **`STEP 3.5b` 的依赖漏洞正则补 `GHSA-`**。此前只写 `CVE-\d{4}-\d+`，而催生这条纪律的
  那次事故（扫描停跑 28 天、漏 4 个漏洞、其中一个明文打印 token）里编号全是 `GHSA-`
  ——**这条检测路径对它要抓的案例从来不可能触发**。
- **`measure.sh` 与 `gen-floor.py` 的归档路径**：两者都硬编码 `.tad/active/` 下已归档的文件，
  归档那天起就跑不起来。`gen-floor.py` 的 `启动扫描` 锚点同时还是 P3 前的旧串。
- **`NEXT.md` 97KB → 5.7KB**，108 条已完成条目迁入 `.tad/archive/next/`（逐字保留）。
  迁移时发现三条标着「最高优先级」的待办**早已修复未划掉**——清单不准即有害。

### 已知限制

- 目标「激活即付 ≤15K tokens」**未达成**（现 62K）。纪律地板本体只占约 20KB，
  其余在「可搬但要搬得安全」的区间，本轮只安全搬动了其中一部分。
- `研究先行` / `技术决策透明` 的本体仍在 Alex 不加载的 `config-cognitive.yaml`；
  两者有常驻祈使句兜底，缺的是展开细节。
- `express` 的专家审查下限在仓库中**存在五个互相矛盾的写法**，尚未统一。
